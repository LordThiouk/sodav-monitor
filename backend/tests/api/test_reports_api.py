...
from backend.utils.auth.auth import create_access_token, get_current_user
...
def test_app_without_auth():
    ...
    from backend.models.database import get_db
    from backend.models.models import Report, ReportStatus, ReportType
    from backend.reports.generator import ReportGenerator
    from backend.routers import auth, channels, reports, websocket
    from backend.routers.analytics import router as analytics_router
    from backend.routers.detections import router as detections_router
    ...
    # Create a custom reports router without global authentication
    custom_reports_router = APIRouter(prefix="/api/reports", tags=["reports"])
    ...
    async def generate_daily_report(
        ...
        try:
            ...
            background_tasks.add_task(generator.generate_daily_report, report.id, date)
    ...
    async def generate_monthly_report(
        ...
        try:
            ...
            generator = ReportGenerator(db)
            background_tasks.add_task(generator.generate_report, report_data)
    ...
    async def get_subscriptions(
        ...
        subscriptions = (
            db.query(ReportSubscription).filter(ReportSubscription.user_id == current_user.id).all()
        )
    ...
    async def delete_subscription(
        subscription_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
    ):
    ...
    async def create_report(
        ...
        try:
            ...
            if "report_type" in data and data["report_type"] not in [
                "daily",
                "weekly",
                "monthly",
                "comprehensive",
...
@pytest.fixture
def test_client_detections(
    db_session: Session, test_user: User, auth_headers: Dict[str, str], test_app_detections
):
...
@pytest.fixture
def test_client_modified_router(
    db_session: Session, test_user: User, auth_headers: Dict[str, str], test_app_modified_router
):
...
def mock_radio_manager():
    ...
    mock = Mock()
    mock.get_station_by_id = Mock(
        return_value={"id": 1, "name": "Test Station", "status": "active"}
    )
...
@pytest.fixture
def test_client_like_detections(
    db_session: Session,
    test_user: User,
    auth_headers: Dict[str, str],
    test_app_like_detections,
    mock_radio_manager,
):
...
@pytest.fixture
def test_client_exact(
    db_session: Session, test_user: User, auth_headers: Dict[str, str], test_app_exact
):
    """Create a test client exactly like in test_detections_api.py."""
    from backend.models.database import get_db
    from backend.utils.auth import get_current_user, oauth2_scheme
...
@pytest.fixture
def test_client_with_router_override(
    db_session: Session,
    test_user: User,
    auth_headers: Dict[str, str],
    test_app_with_router_override,
):
...
def test_get_reports_with_router_override(
    test_client_with_router_override: TestClient, test_report: Report, auth_headers: Dict[str, str]
):
...
class TestReportsAPI:
    ...
    def test_generate_daily_report(
        self, test_client_with_router_override: TestClient, auth_headers: Dict[str, str]
    ):
    ...
    def test_create_subscription(
        ...
        response = test_client_with_router_override.post(
            "/api/reports/subscriptions", json=subscription_data
        )
    ...
    def test_update_subscription(
        ...
        response = test_client_with_router_override.put(
            f"/api/reports/subscriptions/{test_subscription.id}", json=update_data
        )
    ...
    def test_delete_subscription(
        ...
        response = test_client_with_router_override.delete(
            f"/api/reports/subscriptions/{test_subscription.id}", headers=auth_headers
        )
    ...
    def test_send_report_email(
        ...
        response = test_client_with_router_override.post(
            f"/api/reports/{test_report.id}/send", json=email_data, headers=auth_headers
        )
        print(f"Response status: {response.status_code}")
        print(f"Response content: {response.content}")
    ...
    def test_invalid_report_parameters(
        ...
        response = test_client_with_router_override.post(
            "/api/reports/", json=invalid_data, headers=auth_headers
        )
    ...
    def test_invalid_subscription_email(
        ...
        response = test_client_with_router_override.post(
            "/api/reports/subscriptions", json=invalid_data, headers=auth_headers
        )
        assert response.status_code == 422
    ...
    def test_generate_daily_report_custom(
        ...
        response = test_client_with_router_override.post(
            "/api/reports/generate/daily", headers=auth_headers
        )
...
def test_get_reports_with_filters(
    test_client: TestClient, test_report: Report, auth_headers: Dict[str, str]
):
    """Test getting reports with filters."""
    response = test_client.get(
        f"/api/reports/?report_type={test_report.type}", headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
...
def test_get_nonexistent_report(
    test_client_with_router_override: TestClient, auth_headers: Dict[str, str]
):
...
def test_update_report_status(
    ...
    update_data = {"status": ReportStatus.PROCESSING.value, "progress": 50.0}
    response = test_client.put(
        f"/api/reports/{test_report.id}/status", json=update_data, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_filter_reports_by_type(
    test_client: TestClient, test_report: Report, auth_headers: Dict[str, str]
):
...
def test_filter_reports_by_date_range(
    test_client: TestClient, test_report: Report, auth_headers: Dict[str, str]
):
...
def test_report_sorting(test_client: TestClient, test_report: Report, auth_headers: Dict[str, str]):
    ...
    data = response.json()
    if len(data) > 1:
        assert datetime.fromisoformat(data[0]["created_at"]) >= datetime.fromisoformat(
            data[1]["created_at"]
        )
...
def test_report_search(test_client: TestClient, test_report: Report, auth_headers: Dict[str, str]):
    ...
    response = test_client.get(
        f"/api/reports/search?query={test_report.title}", headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
...
def test_report_export_formats(
    ...
    for format in formats:
        response = test_client.get(
            f"/api/reports/{test_report.id}/export?format={format}", headers=auth_headers
        )
        assert response.status_code == 200
        assert response.headers["content-type"] == f"application/{format}"


def test_report_email_validation(
    test_client: TestClient, test_report: Report, auth_headers: Dict[str, str]
):
    ...
    response = test_client.post(
        f"/api/reports/{test_report.id}/send", json=invalid_email_data, headers=auth_headers
    )
    assert response.status_code == 422
...
def test_subscription_frequency_validation(test_client: TestClient, auth_headers: Dict[str, str]):
    ...
    response = test_client.post(
        "/api/reports/subscriptions", json=invalid_subscription, headers=auth_headers
    )
    assert response.status_code == 422


def test_subscription_filters(
    test_client: TestClient, test_subscription: ReportSubscription, auth_headers: Dict[str, str]
):
    ...
    response = test_client.put(
        f"/api/reports/subscriptions/{test_subscription.id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_generate_monthly_report(test_client: TestClient, auth_headers: Dict[str, str]):
    ...
    response = test_client.post(
        f"/api/reports/generate/monthly?year={today.year}&month={today.month}", headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_create_subscription(test_client: TestClient, auth_headers: Dict[str, str]):
    ...
    response = test_client.post(
        "/api/reports/subscriptions", json=subscription_data, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_download_pending_report(
    test_client: TestClient, test_report: Report, auth_headers: Dict[str, str], db_session: Session
):
...
def test_generate_daily_report_with_custom_client(
...
def test_generate_daily_report_like_detections(
    test_client_like_detections, auth_headers: Dict[str, str]
):
...
def test_auth_required(test_app):
    ...
    with TestClient(test_app) as client:
        ...
        ), "Le message d'erreur devrait indiquer que l'utilisateur n'est pas authentifié"


def test_exact_get_reports(
    test_client_exact: TestClient, test_report: Report, auth_headers: Dict[str, str]
):
