...
class TestStatsUpdater(unittest.TestCase):
    ...
    def test_validate_duration(self):
        ...
        result = self.stats_updater._validate_duration(duration)
        self.assertEqual(result, timedelta(seconds=15))  # La méthode retourne 15 secondes par défaut pour les durées négatives
    ...
    @patch('backend.utils.analytics.stats_updater.StatsUpdater._update_temporal_aggregates_efficient')
    ...
    def test_update_all_stats_calls_all_methods(
        self, mock_update_station, mock_update_analytics, 
        mock_update_temporal, mock_update_detection
    ):
