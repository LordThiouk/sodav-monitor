...
class TestEndToEnd:
        ...
        for station in available_stations:
            ...
            try:
                ...
                if audio_data is None or duration <= 0:
                    logger.warning(
                        f"Failed to capture audio from {station.name}, trying next station"
                    )
                    continue
                ...
                logger.info(
                    f"Audio from {station.name} classified as {'music' if is_music else 'speech'}"
                )

                if not is_music:
                    logger.info(
                        f"Audio from {station.name} classified as speech, trying next station"
                    )
                    continue

                # 4. Perform detection
                detection_result = await track_manager.process_track(
                    features=features, station_id=station.id
                )
                ...
                if "track_id" in detection_result:
                    ...
                    assert track_stats.total_play_time > timedelta(
                        seconds=0
                    ), "Play time not recorded in stats"
                    ...
                    assert station_track_stats.total_play_time > timedelta(
                        seconds=0
                    ), "Play time not recorded in station stats"

                    logger.info(
                        f"Detection workflow test passed with match for station: {station.name}"
                    )
                else:
                    ...
                    logger.info(
                        f"No match found for station: {station.name}, but detection process completed successfully"
                    )
                    logger.info(
                        "Note: In test environments, external detection services (AcoustID and AudD) often fail with 'Failed to convert features to audio' errors. This is expected behavior."
                    )
    ...
    @pytest.mark.asyncio
    async def test_play_duration_accuracy(
        self, db_session, test_stations, track_manager, feature_extractor
    ):
        """
        ...
        logger.info(
            f"Expected duration: {expected_duration:.2f}s, Recorded duration: {recorded_duration:.2f}s"
        )
        assert (
            abs(recorded_duration - expected_duration) <= tolerance
    ...
    async def test_performance_and_scalability(
        ...
        for station, audio_data, _ in station_audio_data:
            ...
            if features is not None and feature_extractor.is_music(features):
                task = track_manager.process_track(features=features, station_id=station.id)
                tasks.append(task)
        ...
        assert (
            avg_time_per_detection <= PERFORMANCE_THRESHOLD
    ...
    async def test_database_consistency(
        ...
        recent_detections = (
            db_session.query(func.count(TrackDetection.id))
            .filter(
                TrackDetection.track_id == track_id,
                TrackDetection.station_id == station.id,
        ...
        assert (
            recent_detections <= 2
        ), f"Found {recent_detections} recent detections, expected 1 or 2"
        ...
        station_track_stats = (
            db_session.query(StationTrackStats)
            .filter(
                StationTrackStats.track_id == track_id, StationTrackStats.station_id == station.id
            )
            .first()
        )
        ...
        detection = (
            db_session.query(TrackDetection).filter(TrackDetection.track_id == track_id).first()
        )
    ...
    async def test_end_to_end_workflow(
        ...
        for station in stations:
            ...
            if "track_id" in detection_result:
                ...
                track_detection = (
                    db_session.query(TrackDetection)
                    .filter(
                        TrackDetection.station_id == station.id, TrackDetection.track_id == track_id
                    )
                    .order_by(TrackDetection.detected_at.desc())
                    .first()
                )
                ...
                assert (
                    track_detection is not None
                ), f"No track detection record found for {station.name}"
                ...
                ), f"Station track stats not updated for {station.name}"

                successful_detections += 1
            else:
                logger.info(
                    f"No track match found for {station.name}, but detection process completed"
                )
        ...
        logger.info(
            f"End-to-end workflow test passed with {successful_detections} successful detections"
        )
