...
from backend.models.models import Track, Artist, RadioStation, TrackDetection, StationTrackStats, Base
...
class TestPlayDurationRealData:
    ...
    def capture_audio_stream(self, stream_url, duration=RECORDING_DURATION, detect_silence=False):
        ...
        try:
            ...
            bytes_to_capture = int(duration * 128 * 1024 / 8) if not detect_silence else float('inf')
    ...
    def test_available_stations(self):
        ...
        for i, station in enumerate(stations, 1):
            ...
            try:
                ...
                status = "Accessible" if response.status_code == 200 else f"Non accessible (code {response.status_code})"
            except Exception as e:
    ...
    async def test_real_radio_duration_capture(self, db_session, test_stations):
        ...
        for station in test_stations[:5]:
            ...
            if audio_data:
                ...
                print(f"Impossible de capturer l'audio de {station.name}, essai avec une autre station")
        ...
        assert station_track_stats is not None, "Les statistiques de la station n'ont pas été créées"
        assert station_track_stats.total_play_time > timedelta(0), "La durée totale de lecture n'a pas été mise à jour"
        
        # Vérifier que la durée totale de lecture dans les statistiques correspond à la durée capturée
        assert abs(station_track_stats.total_play_time.total_seconds() - captured_duration_td.total_seconds()) < 0.5, \
        ...
        print(f"Test réussi! Morceau détecté: {detection.track.title} par {detection.track.artist.name}")
        print(f"Durée enregistrée: {detection.play_duration} secondes")
        print(f"Méthode de détection: {detection.detection_method}")
        print(f"Confiance: {detection.confidence}")
        print(f"Statistiques: {station_track_stats.play_count} lecture(s), {station_track_stats.total_play_time} au total")
    ...
    async def test_multiple_stations_duration_comparison(self, db_session, test_stations):
        ...
        for station in test_stations:
            ...
            print(f"Capture d'un extrait audio de {station.name} pendant {station_recording_duration} secondes...")
            audio_data, captured_duration = self.capture_audio_stream(station.stream_url, duration=station_recording_duration)
            
            if not audio_data:
                print(f"Impossible de capturer l'audio de {station.name}, passage à la station suivante")
                continue
            ...
            if "error" in result:
                ...
                if result["error"] == "No match found for track":
                    ...
                    # Vérifier que la durée enregistrée correspond à la durée capturée (avec une marge d'erreur de 0.5 seconde)
                    assert abs(detection.play_duration.total_seconds() - captured_duration) < 0.5, \
                    ...
                    assert station_track_stats.total_play_time > timedelta(0), f"La durée totale de lecture pour {station.name} n'a pas été mise à jour"
                    
                    # Vérifier que la durée totale de lecture dans les statistiques correspond à la durée capturée
                    assert abs(station_track_stats.total_play_time.total_seconds() - captured_duration) < 0.5, \
                        f"La durée totale de lecture dans les statistiques ({station_track_stats.total_play_time.total_seconds()} s) " \
            ...
            if detection is None:
                print(f"La détection n'a pas été enregistrée pour {station.name}, passage à la station suivante")
                continue
        ...
        if len(results) >= 2:
            ...
            for i in range(len(station_names)):
                for j in range(i+1, len(station_names)):
                    ...
                    diff = abs(results[station1]["recorded_duration"] - results[station2]["recorded_duration"])
    ...
    async def test_real_radio_full_detection_cycle(self, db_session, test_stations):
        ...
        with patch('backend.detection.audio_processor.track_manager.track_finder.TrackFinder.find_local_match', 
             ...
             patch('backend.detection.audio_processor.track_manager.track_finder.TrackFinder.find_track_by_isrc', 
            ...
            if "error" in result:
                ...
                if result["error"] == "No match found for track":
                    ...
                    # Créer ou mettre à jour les statistiques de la station
                    track_stats = db_session.query(StationTrackStats).filter(
                        StationTrackStats.station_id == test_station.id,
                        StationTrackStats.track_id == unknown_track.id
                    ).first()
        ...
        assert track_stats.total_play_time > timedelta(0), "La durée totale de lecture n'a pas été mise à jour"
        
        # Vérifier que la durée totale de lecture dans les statistiques correspond à la durée capturée
        captured_duration_td = timedelta(seconds=captured_duration)
        assert abs(track_stats.total_play_time.total_seconds() - captured_duration_td.total_seconds()) < 0.5, \
    ...
    async def test_real_radio_duration_until_silence(self, db_session, test_stations):
        ...
        assert abs(features["duration"] - captured_duration) < 0.5, \
        ...
        # Vérifier que la durée totale de lecture dans les statistiques correspond à la durée capturée
        assert abs(station_track_stats.total_play_duration.total_seconds() - captured_duration) < 0.5, \
