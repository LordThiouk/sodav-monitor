...
class TestDetectionPipeline:
    ...
    def test_station(self, db_session: Session) -> RadioStation:
        ...
        station = db_session.query(RadioStation).filter(RadioStation.name == "Test Integration Station").first()
        if not station:
    ...
    @pytest.mark.asyncio
    async def test_local_detection(self, db_session: Session, test_station: RadioStation, 
                                  test_track: Track, mock_audio_data: bytes):
        """
        ...
        with patch.object(StreamHandler, 'get_audio_data', return_value=mock_audio_data):
            ...
            with patch.object(AudioProcessor, 'process_stream', mock_process_stream):
                ...
                assert result["details"]["track"]["id"] == test_track.id, "Did not detect the correct track"
    ...
    @pytest.mark.asyncio
    async def test_hierarchical_detection(self, db_session: Session, test_station: RadioStation,
        ...
        if not artist:
            ...
            artist = Artist(
                ...
                country="US",
                label="MusicBrainz Test Label"
            )
            db_session.add(artist)
            db_session.commit()

        # Check if the track already exists
        track = db_session.query(Track).filter_by(title="MusicBrainz Test Track", artist_id=artist.id).first()
        if not track:
    ...
    @pytest.mark.asyncio
    async def test_speech_detection(self, db_session: Session, test_station: RadioStation, 
                                   mock_audio_data: bytes):
        """
        ...
        with patch.object(StreamHandler, 'get_audio_data', return_value=mock_audio_data):
            ...
            with patch.object(AudioProcessor, 'process_stream', mock_process_stream):
                ...
                assert final_detection_count == initial_detection_count, "Detection was incorrectly saved for speech" 