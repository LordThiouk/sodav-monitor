...
class TestContinuousPlayDuration:
    ...
    def test_stations(self, db_session):
        ...
        stations = []
        for i, station_data in enumerate(
            senegal_stations[:5]
        ):  # Limiter à 5 stations pour les tests
            station = RadioStation(
                name=station_data["name"], stream_url=station_data["url"], status="active"
            )
    ...
    def capture_audio_stream_until_silence(
        self,
        stream_url,
        ...
        try:
            ...
            for chunk in response.iter_content(chunk_size=segment_size):
                if chunk:
                    ...
                    if len(segments_buffer) >= 5 and bytes_captured % (segment_size * 5) == 0:
                        ...
                        try:
                            ...
                            if normalized_rms < silence_threshold:
                                ...
                                logger.debug(
                                    f"Silence détecté: {silence_duration:.2f}s (seuil: {min_silence_duration:.2f}s)"
                                )
                            ...
                            if previous_segment is not None:
                                ...
                                if len(current_spectrum) > 0 and len(previous_spectrum) > 0:
                                    ...
                                    logger.debug(
                                        f"Différence spectrale: {spectral_diff:.4f} (seuil: {spectral_change_threshold:.4f})"
                                    )
                    ...
                    if elapsed > max_capture_duration:
                        logger.warning(
                            f"Durée maximale de capture atteinte ({max_capture_duration} secondes)"
                        )
            ...
            logger.info(
                f"Capture terminée après {total_elapsed:.2f} secondes. Raison: {end_reason}"
            )
    ...
    async def test_silence_detection(self, db_session, test_stations):
        ...
        logger.info(
            f"Test de détection de silence avec la station: {station.name} ({station.stream_url})"
        )
        ...
        logger.info(
            f"Audio capturé avec succès. Durée: {captured_duration:.2f} secondes. Raison de fin: {end_reason}"
        )

        # Vérifier que la capture s'est terminée à cause d'un silence détecté
        if end_reason != "silence_detected":
            logger.warning(
                f"La capture ne s'est pas terminée à cause d'un silence détecté, mais pour la raison: {end_reason}"
            )
        ...
        try:
            ...
            assert (
                abs(features["duration"] - captured_duration) < 1.0
            ...
            ), "La détection n'a pas été enregistrée dans la base de données"
            ...
            assert (
                abs(station_track_stats.total_play_duration.total_seconds() - captured_duration)
                < 1.0
            ), (
    ...
    async def test_spectral_change_detection(self, db_session, test_stations):
        ...
        logger.info(
            f"Test de détection de changement spectral avec la station: {station.name} ({station.stream_url})"
        )
    ...
    async def test_multiple_stations_continuous_duration(self, db_session, test_stations):
        ...
        for station in selected_stations:
            ...
            audio_data, captured_duration, end_reason = self.capture_audio_stream_until_silence(
                station.stream_url, max_capture_duration=90  # Limiter à 90 secondes pour ce test
            )
            ...
            try:
                ...
                if not features:
                    logger.warning(
                        f"Impossible d'extraire les caractéristiques de l'audio pour {station.name}"
                    )
                    continue
                ...
                if not result["success"]:
                    logger.warning(
                        f"Le traitement a échoué pour {station.name}: {result.get('error', 'Erreur inconnue')}"
                    )
                    continue
                ...
                if not detection:
                    logger.warning(
                        f"La détection n'a pas été enregistrée dans la base de données pour {station.name}"
                    )
                    continue
        ...
        assert (
            len(station_results) >= 2
        ), f"Pas assez de stations avec des résultats valides ({len(station_results)})"
        ...
        for result in station_results:
            assert (
                abs(result["recorded_duration"] - result["captured_duration"]) < 1.0
    ...
    async def test_safety_limit(self, db_session, test_stations):
        ...
        station = test_stations[0]
        logger.info(
            f"Test de limite de sécurité avec la station: {station.name} ({station.stream_url})"
        )
        ...
        assert (
            end_reason == "max_duration_reached"
        ), f"La capture ne s'est pas terminée à cause de la limite de sécurité, mais pour la raison: {end_reason}"
