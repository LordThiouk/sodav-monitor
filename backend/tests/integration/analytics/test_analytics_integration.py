...
class TestAnalyticsIntegration:
    ...
    def test_stats_calculation(self, db_session: Session):
        ...
        station = (
            ...
            .filter(RadioStation.name == "Analytics Test Station")
            .first()
        )
        if not station:
        ...
        if not artist:
            artist = Artist(
                name="Analytics Test Artist", country="FR", label="Analytics Test Label"
            )
        ...
        artist_stats = (
            db_session.query(ArtistStats).filter(ArtistStats.artist_id == artist.id).first()
        )
    ...
    def test_analytics_data_generation(self, db_session: Session):
        ...
        station = (
            db_session.query(RadioStation)
            .filter(RadioStation.name == "Analytics Data Test Station")
            .first()
        )
        if not station:
            station = RadioStation(
        ...
        # Create a test artist
        artist = (
            db_session.query(Artist).filter(Artist.name == "Analytics Data Test Artist").first()
        )
        if not artist:
            artist = Artist(
                name="Analytics Data Test Artist", country="FR", label="Analytics Data Test Label"
            )
            db_session.add(artist)
            db_session.commit()
            db_session.refresh(artist)
