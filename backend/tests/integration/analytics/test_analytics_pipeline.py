...
from backend.models.models import (
    ...
    TrackStats, ArtistStats, StationStats, StationTrackStats,
    AnalyticsData
)
...
class TestAnalyticsPipeline:
    ...
    def test_track_stats_update(self, db_session: Session, test_track: Track, test_detection: TrackDetection):
        """
        ...
        track_stats = db_session.query(TrackStats).filter(TrackStats.track_id == test_track.id).first()
        ...
        assert track_stats.last_detected == test_detection.detected_at, "Last detected time not updated"
    
    def test_artist_stats_update(self, db_session: Session, test_artist: Artist, test_detection: TrackDetection):
        """
        ...
        artist_stats = db_session.query(ArtistStats).filter(ArtistStats.artist_id == test_artist.id).first()
        ...
        assert artist_stats.last_detected == test_detection.detected_at, "Last detected time not updated"
    
    def test_station_stats_update(self, db_session: Session, test_station: RadioStation, test_detection: TrackDetection):
        """
        ...
        station_stats = db_session.query(StationStats).filter(StationStats.station_id == test_station.id).first()
        ...
        assert station_stats.last_detected == test_detection.detected_at, "Last detected time not updated"
    
    def test_station_track_stats_update(self, db_session: Session, test_station: RadioStation, 
                                       test_track: Track, test_detection: TrackDetection):
        """
        ...
        assert station_track_stats.total_play_time == timedelta(seconds=180), "Total play time not updated"
        
        # Compare only the timestamp parts, ignoring timezone info
        assert station_track_stats.last_played.replace(tzinfo=None) == test_detection.detected_at.replace(tzinfo=None), "Last played time not updated"
    
    def test_multiple_detections_stats(self, db_session: Session, test_track: Track, 
                                      test_station: RadioStation, test_detection: TrackDetection):
        """
        ...
        assert track_stats.total_play_time == timedelta(seconds=180 + 120*2), "Track total play time not updated correctly"
        
        # Verify artist statistics
        artist_stats = db_session.query(ArtistStats).filter(ArtistStats.artist_id == test_track.artist_id).first()
        assert artist_stats.total_plays == 3, "Artist detection count not updated correctly"
        assert artist_stats.total_play_time == timedelta(seconds=180 + 120*2), "Artist total play time not updated correctly"
        ...
        assert station_track_stats.total_play_time == timedelta(seconds=180 + 120*2), "Station-track total play time not updated correctly"
    
    def test_analytics_data_generation(self, db_session: Session, test_track: Track, 
                                      test_station: RadioStation, test_detection: TrackDetection):
        """
