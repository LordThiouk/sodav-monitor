...
@pytest.mark.asyncio
async def test_start_monitoring_success(
    station_monitor, sample_station, mock_health_check, db_session
):
    ...
    # Mock health check
    with patch.object(
        station_monitor.stream_checker, "check_stream_availability", return_value=mock_health_check
    ):
...
async def test_start_monitoring_stream_unavailable(station_monitor, sample_station, db_session):
    ...
    with patch.object(
        station_monitor.stream_checker, "check_stream_availability", return_value=mock_health
    ):
        success = await station_monitor.start_monitoring(sample_station.id)
...
async def test_check_stream_health_error(station_monitor):
    ...
    with patch.object(
        station_monitor.stream_checker,
...
@pytest.mark.asyncio
async def test_update_station_health_healthy(
    station_monitor, sample_station, mock_health_check, db_session
):
...
async def test_update_station_health_degraded(station_monitor, sample_station, db_session):
    ...
    health_data = {"is_available": True, "is_audio_stream": True, "latency": 5000}  # High latency
...
@pytest.mark.asyncio
async def test_handle_station_recovery_success(
    station_monitor, sample_station, mock_health_check, db_session
):
