...
async def test_musicbrainz_successful_detection(mock_musicbrainz_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post(
            ...
            payload={
                "results": [
                    {"id": "test_id", "title": "Test Track", "artist": "Test Artist", "score": 0.95}
                ]
            },
        )
...
async def test_musicbrainz_no_results(mock_musicbrainz_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post(
            "https://api.musicbrainz.org/v1/lookup", status=200, payload={"results": []}
        )
...
async def test_musicbrainz_api_error(mock_musicbrainz_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post(
            "https://api.musicbrainz.org/v1/lookup", status=500, body="Internal Server Error"
        )
...
async def test_audd_successful_detection(mock_audd_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post(
            ...
            payload={
                "status": "success",
                "result": {"title": "Test Track", "artist": "Test Artist", "score": 0.85},
            },
        )
...
async def test_audd_no_match(mock_audd_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post(
            "https://api.audd.io/", status=200, payload={"status": "success", "result": None}
        )
...
async def test_audd_api_error(mock_audd_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post("https://api.audd.io/", status=500, body="Internal Server Error")
...
async def test_musicbrainz_timeout(mock_musicbrainz_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post("https://api.musicbrainz.org/v1/lookup", timeout=True)
...
async def test_audd_timeout(mock_audd_service):
    ...
    with aioresponses() as mock_response:
        mock_response.post("https://api.audd.io/", timeout=True)
