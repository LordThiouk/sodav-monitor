...
from backend.models.models import Track, TrackDetection, Artist
...
async def test_save_fingerprint(detector, mock_audio_data):
    ...
    with patch.object(detector.fingerprinter, 'generate_fingerprint', 
                     return_value=(180.0, "test_fingerprint", {})):
...
async def test_search_local_with_cache(detector, mock_audio_data):
    ...
    with patch.object(detector, '_get_cached_fingerprint', 
                     return_value=(180.0, "test_fingerprint")):
        # Mock database query
        mock_query = MagicMock()
        mock_query.all.return_value = [(mock_track, "test_fingerprint", 1)]
        detector.db_session.query.return_value.join.return_value.group_by.return_value\
            .order_by.return_value.limit.return_value = mock_query
            
        # Mock fingerprint comparison
        with patch.object(detector.fingerprinter, 'compare_fingerprints', 
                         return_value=0.9):
...
async def test_search_local_low_confidence(detector, mock_audio_data):
    ...
    with patch.object(detector.fingerprinter, 'generate_fingerprint', 
        ...
        with patch.object(detector.fingerprinter, 'compare_fingerprints', 
                         return_value=0.7):  # Below 0.8 threshold
            result = await detector.search_local(mock_audio_data)
            assert result is None
