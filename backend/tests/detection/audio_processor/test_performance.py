...
def large_audio_data():
    ...
    signal = (
        ...
        + np.sin(2 * np.pi * 880 * t)  # A4 note
        + np.sin(2 * np.pi * 1760 * t)  # A5 note
    )  # A6 note
    return signal
...
@pytest.mark.benchmark(group="audio_processor", min_rounds=100, max_time=2.0)
...
@pytest.mark.benchmark(group="audio_processor", min_rounds=5, max_time=5.0)
...
@pytest.mark.benchmark(group="audio_processor", min_rounds=10, max_time=2.0)
