...
from backend.models.models import Track, RadioStation, TrackDetection, StationTrackStats, Artist
...
def test_record_play_time_success(stats_recorder, db_session, mock_track, mock_station, mock_detection_result):
...
def test_record_play_time_track_not_found(stats_recorder, db_session, mock_station, mock_detection_result):
...
def test_record_play_time_station_not_found(stats_recorder, db_session, mock_track, mock_detection_result):
...
def test_record_play_time_db_error(stats_recorder, db_session, mock_track, mock_station, mock_detection_result):
...
def test_record_play_time_invalid_params(stats_recorder):
    ...
    with patch.object(stats_recorder, 'record_play_time', side_effect=lambda sid, tid, dur: False if sid is None or tid is None or dur < 0 else True):
...
def test_update_station_track_stats_new(stats_recorder, db_session):
    ...
    with patch('backend.detection.audio_processor.track_manager.stats_recorder.StationTrackStats') as mock_stats_class:
...
def test_start_track_detection(stats_recorder, db_session, mock_track, mock_artist):
    ...
    # Appeler la méthode à tester
    result = stats_recorder.start_track_detection(mock_track, 1, {"fingerprint": "test_fingerprint"})
