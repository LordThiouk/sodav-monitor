...
def external_detection(config):
    ...
    with patch('backend.detection.audio_processor.track_manager.external_detection.get_settings', return_value=config):
...
@pytest.mark.asyncio
async def test_detect_with_audd_no_result(external_detection, mock_audio_data, mock_audd_no_result_response):
...
@pytest.mark.asyncio
async def test_detect_with_audd_error(external_detection, mock_audio_data, mock_audd_error_response):
    ...
    with patch.object(external_detection, '_parse_audd_result') as mock_parse:
        ...
        with patch('aiohttp.ClientSession', return_value=mock_session), \
             patch.object(external_detection.logger, 'error') as mock_logger:
            # Appeler la méthode à tester
            result = await external_detection.detect_with_audd(mock_audio_data)
...
@pytest.mark.asyncio
async def test_detect_with_acoustid_success(external_detection, mock_audio_data, mock_acoustid_response):
...
@pytest.mark.asyncio
async def test_detect_with_acoustid_no_result(external_detection, mock_audio_data, mock_acoustid_no_result_response):
...
@pytest.mark.asyncio
async def test_detect_with_acoustid_error(external_detection, mock_audio_data, mock_acoustid_error_response):
...
async def test_detect_music_audd_success(external_detection, mock_audio_data, mock_audd_response):
    ...
    with patch.object(external_detection, 'detect_with_audd', return_value={"track": {"title": "Test Track"}, "confidence": 0.9}), \
...
@pytest.mark.asyncio
async def test_detect_music_acoustid_success(external_detection, mock_audio_data, mock_acoustid_response):
    """Teste la détection musicale avec AcoustID réussi après échec AudD."""
    # Patch les méthodes de détection
    with patch.object(external_detection, 'detect_with_audd', return_value=None), \
         patch.object(external_detection, 'detect_with_acoustid', return_value={"track": {"title": "Test Track"}, "confidence": 0.9}):
...
def test_create_acoustid_signature(external_detection):
    ...
    with patch('hmac.new') as mock_hmac, \
         patch('base64.b64encode', return_value=b"test_signature"):
