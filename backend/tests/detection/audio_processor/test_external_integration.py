...
class TestMusicBrainzIntegration:
    ...
    @pytest.mark.asyncio
    async def test_musicbrainz_match_success(
        self, track_manager, mock_external_handler, mock_db_session, sample_audio_features
    ):
        """Test successful MusicBrainz match."""
        # Mock the external handler
        with patch(
            "backend.detection.audio_processor.track_manager.ExternalServiceHandler",
            return_value=mock_external_handler,
        ):
            ...
            mock_db_session.add.side_effect = (
                lambda x: setattr(x, "id", 1) if isinstance(x, Artist) else None
            )
            ...
            mock_external_handler.recognize_with_musicbrainz.assert_called_once_with(
                b"mock_audio_data"
            )

    @pytest.mark.asyncio
    async def test_musicbrainz_match_failure(
        self, track_manager, mock_external_handler, sample_audio_features
    ):
    ...
    @pytest.mark.asyncio
    async def test_musicbrainz_error_handling(
        self, track_manager, mock_external_handler, sample_audio_features
    ):
...
class TestAuddIntegration:
    ...
    @pytest.mark.asyncio
    async def test_audd_match_success(
        self, track_manager, mock_external_handler, mock_db_session, sample_audio_features
    ):
        ...
        with patch(
            ...
            mock_db_session.add.side_effect = (
                lambda x: setattr(x, "id", 2) if isinstance(x, Artist) else None
            )
    ...
    @pytest.mark.asyncio
    async def test_audd_match_failure(
        self, track_manager, mock_external_handler, sample_audio_features
    ):
    ...
    @pytest.mark.asyncio
    async def test_audd_error_handling(
        self, track_manager, mock_external_handler, sample_audio_features
    ):
...
class TestHierarchicalDetectionWithExternalServices:
    ...
    @pytest.mark.asyncio
    async def test_full_detection_chain(
        self, track_manager, mock_db_session, sample_audio_features
    ):
    ...
    @pytest.mark.asyncio
    async def test_detection_chain_with_musicbrainz_match(
        self, track_manager, mock_db_session, sample_audio_features
    ):
        ...
        # Mock the methods
        track_manager.find_local_match = AsyncMock(return_value=None)
        track_manager.find_musicbrainz_match = AsyncMock(
            return_value={"track": mock_track, "confidence": 0.85, "source": "musicbrainz"}
        )
    ...
    @pytest.mark.asyncio
    async def test_detection_chain_with_audd_match(
        self, track_manager, mock_db_session, sample_audio_features
    ):
        ...
        track_manager.find_musicbrainz_match = AsyncMock(return_value=None)
        track_manager.find_audd_match = AsyncMock(
            return_value={"track": mock_track, "confidence": 0.8, "source": "audd"}
        )
