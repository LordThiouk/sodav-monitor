...
async def test_find_audd_match_success(track_manager):
    ...
    mock_response = {
        "status": "success",
        "result": {"title": "Audd Track", "artist": "Audd Artist", "score": 0.8},
    }
...
async def test_find_audd_match_no_match(track_manager):
    ...
    mock_response = {"status": "success", "result": None}
...
@pytest.mark.asyncio
async def test_save_detection_database_error(
    track_manager, sample_track, sample_station, db_session
):
