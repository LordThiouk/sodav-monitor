...
def create_test_stations(db_session: Session, num_stations: int = 10):
    ...
    stations = db_session.query(RadioStation).filter(RadioStation.name.like("Test Station%")).all()
...
def test_station_monitoring_performance(test_client: TestClient, db_session: Session):
    ...
    for count in station_counts:
        ...
        # Fetch all test stations
        stations = (
            db_session.query(RadioStation).filter(RadioStation.name.like("Test Station%")).all()
        )
        ...
        response = test_client.post("/api/channels/detect-music", params={"max_stations": count})
    ...
    if processing_times and stations_per_second:
        ...
        logger.info(
            f"Estimated Optimal Number of Stations for 60-second processing: {estimated_optimal_stations}"
        )
...
async def test_concurrent_station_processing(db_session: Session):
    ...
    for concurrency in concurrency_levels:
        ...
        async def process_station(station):
            async with sem:
                ...
                return {"station_id": station.id, "station_name": station.name, "status": "success"}
    ...
    if processing_times and stations_per_second:
        ...
        logger.info(
            f"Optimal Processing Time: {results[optimal_concurrency]['processing_time']:.2f} seconds"
        )
        logger.info(
            f"Optimal Stations Per Second: {results[optimal_concurrency]['stations_per_second']:.2f}"
        )
...
def test_system_resource_usage_during_monitoring(test_client: TestClient, db_session: Session):
    ...
    memory_usage_per_station = {
        count: data["memory_usage"] / count for count, data in results.items()
    }
    ...
    for count, cpu_usage in cpu_usage_per_station.items():
        ...
        logger.info(
            f"Memory Usage Per Station: {memory_usage_per_station[count] / (1024 * 1024):.4f} MB"
        )
        logger.info("---")
    ...
    max_stations_by_cpu = (
        int(available_cpu / avg_cpu_per_station) if avg_cpu_per_station > 0 else float("inf")
    )
    max_stations_by_memory = (
        int(available_memory / avg_memory_per_station)
        if avg_memory_per_station > 0
        else float("inf")
    )
    ...
    logger.info(
        f"Estimated Maximum Number of Stations Based on Available Resources: {max_stations}"
    )
