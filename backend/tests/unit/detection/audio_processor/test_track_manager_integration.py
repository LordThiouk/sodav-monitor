...
async def test_track_creation_integration(mock_db_session, mock_track):
    ...
    with patch.object(
        track_manager.track_creator, "get_or_create_artist", new_callable=AsyncMock, return_value=1
    ), patch.object(
        track_manager.track_creator,
        "get_or_create_track",
...
async def test_local_detection_integration(mock_db_session, mock_track, mock_features):
    ...
    with patch.object(
        ...
        mock_record_detection.return_value = {"success": True, "track_id": 1, "detection_id": 1}
...
async def test_external_detection_integration(mock_db_session, mock_track, mock_features):
    ...
    with patch.object(
        ...
        mock_start_detection.return_value = {"success": True, "track_id": 1, "detection_id": 1}
...
async def test_fingerprint_integration(mock_db_session, mock_features):
    ...
    with patch.object(
        track_manager.fingerprint_handler, "extract_fingerprint"
    ) as mock_extract_fingerprint:
        # Configure the mock
...
async def test_stats_recording_integration(mock_db_session, mock_track, mock_station):
    ...
    with patch.object(
        track_manager.stats_recorder, "record_detection"
    ) as mock_record_detection, patch.object(
        track_manager.stats_recorder, "start_track_detection"
    ) as mock_start_detection, patch.object(
        track_manager.stats_recorder, "record_play_time"
    ) as mock_record_play_time:
        # Configure the mocks
        mock_record_detection.return_value = {"success": True, "track_id": 1, "detection_id": 1}
        ...
        result2 = track_manager.stats_recorder.start_track_detection(
            mock_track, 1, {"confidence": 0.9, "detection_method": "local"}
        )
        ...
        mock_start_detection.assert_called_once_with(
            mock_track, 1, {"confidence": 0.9, "detection_method": "local"}
        )
...
@pytest.mark.asyncio
async def test_full_detection_pipeline_integration(
    mock_db_session, mock_track, mock_features, mock_station
):
    """
