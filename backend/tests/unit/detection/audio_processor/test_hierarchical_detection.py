...
def mock_track():
    ...
    track.artist_id = 1
    track.to_dict = Mock(return_value={"id": 1, "title": "Test Track", "artist": "Test Artist"})
    return track
...
def mock_feature_extractor():
    ...
    extractor.extract_features = Mock(
        return_value={
            "is_music": True,
            "confidence": 0.85,
            "play_duration": 5.0,
...
@pytest.fixture
def audio_processor(
    mock_db_session, mock_feature_extractor, mock_track_manager, mock_stream_handler
):
...
class TestHierarchicalDetection:
    ...
    @pytest.mark.asyncio
    async def test_local_detection_success(
        self, audio_processor, mock_track_manager, mock_track, sample_audio_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_musicbrainz_detection_fallback(
        self, audio_processor, mock_track_manager, mock_track, sample_audio_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_audd_detection_fallback(
        self, audio_processor, mock_track_manager, mock_track, sample_audio_data
    ):
...
class TestPlayDurationTracking:
    ...
    @pytest.mark.asyncio
    async def test_play_duration_included_in_result(
        self, audio_processor, mock_feature_extractor, sample_audio_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_track_manager_receives_duration(
        self, audio_processor, mock_track_manager, mock_track, sample_audio_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_continuous_playback_accumulation(
        self, audio_processor, mock_track_manager, mock_track, sample_audio_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_track_change_resets_duration(
        self, audio_processor, mock_track_manager, mock_track, sample_audio_data
    ):
