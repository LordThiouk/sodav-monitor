...
def test_station_stats(db_session: Session, test_station: RadioStation) -> StationStats:
    ...
    # First check if stats already exist
    existing_stats = (
        db_session.query(StationStats).filter(StationStats.station_id == test_station.id).first()
    )
...
class TestAnalyticsAPI:
    ...
    async def test_get_trend_analysis(
        ...
        response = test_client.get(
            "/api/analytics/dashboard", params={"period": 24}, headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_stats_by_timeframe(
        ...
        response = test_client.get(
            "/api/analytics/dashboard", params={"period": 168}, headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_invalid_timeframe(
        self, test_client: TestClient, auth_headers: Dict[str, str], stats_manager: StatsManager
    ):
        """Test invalid timeframe parameter."""
        response = test_client.get(
            "/api/analytics/dashboard", params={"period": -1}, headers=auth_headers
        )
