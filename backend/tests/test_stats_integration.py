...
from sqlalchemy import create_engine, text, Column, Integer, String, ForeignKey, DateTime, Interval, LargeBinary, Float, Boolean
from sqlalchemy.orm import sessionmaker, Session, relationship
from sqlalchemy.ext.declarative import declarative_base
from backend.models.models import (
    Track, Artist, TrackDetection, TrackStats, ArtistStats,
    StationTrackStats, RadioStation
)
...
class TestStatsIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.original_update_detection_stats_efficient = StatsUpdater._update_detection_stats_efficient
        self.original_update_temporal_aggregates_efficient = StatsUpdater._update_temporal_aggregates_efficient
        self.original_update_analytics_data_efficient = StatsUpdater._update_analytics_data_efficient
        self.original_update_station_status_efficient = StatsUpdater._update_station_status_efficient
        
        # Replace with mock methods
        StatsUpdater._update_detection_stats_efficient = self.mock_update_detection_stats_efficient
        StatsUpdater._update_temporal_aggregates_efficient = self.mock_update_temporal_aggregates_efficient
    ...
    def tearDown(self):
        ...
        StatsUpdater._update_detection_stats_efficient = self.original_update_detection_stats_efficient
        StatsUpdater._update_temporal_aggregates_efficient = self.original_update_temporal_aggregates_efficient
        StatsUpdater._update_analytics_data_efficient = self.original_update_analytics_data_efficient
        StatsUpdater._update_station_status_efficient = self.original_update_station_status_efficient
    ...
    def test_record_play_time_updates_stats(self):
        ...
        track_stats_before = self.session.query(TrackStats).filter_by(track_id=self.track_id).first()
        artist_stats_before = self.session.query(ArtistStats).filter_by(artist_id=self.artist_id).first()
        station_track_stats_before = self.session.query(StationTrackStats).filter_by(
        ...
        artist_stats_after = self.session.query(ArtistStats).filter_by(artist_id=self.artist_id).first()
        station_track_stats_after = self.session.query(StationTrackStats).filter_by(
            station_id=self.station_id, track_id=self.track_id
        ).first()
    ...
    def test_multiple_detections_accumulate_stats(self):
        ...
        self.assertAlmostEqual(
            artist_stats.total_play_time.total_seconds(),
            sum(play_durations),
            delta=1.0
        )
    ...
    def test_detection_record_created(self):
        ...
        detection = self.session.query(TrackDetection).filter_by(
            station_id=self.station_id, track_id=self.track_id
        ).first()
        ...
        self.assertAlmostEqual(
            detection.play_duration.total_seconds(),
            play_duration,
            delta=1.0
        )

    def mock_update_detection_stats_efficient(self, station_id, track_id, artist_id, 
                                             confidence, play_duration, current_time):
        ...
        if track_stats:
            ...
            track_stats.average_confidence = ((track_stats.average_confidence * (track_stats.total_plays - 1)) + confidence) / track_stats.total_plays
        else:
        ...
        if artist_stats:
            ...
            artist_stats.average_confidence = ((artist_stats.average_confidence * (artist_stats.total_plays - 1)) + confidence) / artist_stats.total_plays
        else:
        ...
        station_track_stats = self.session.query(TestStationTrackStats).filter_by(
            station_id=station_id, track_id=track_id).first()
        if station_track_stats:
            ...
            station_track_stats.average_confidence = ((station_track_stats.average_confidence * (station_track_stats.play_count - 1)) + confidence) / station_track_stats.play_count
        else:
    ...
    def mock_record_play_time(self, station_id, track_id, play_duration, confidence=0.8, detection_method='audd'):
        ...
        try:
            ...
            self.session.commit()
            print(f"Recorded play time for track ID {track_id} on station ID {station_id}: {play_duration} seconds")
            return True
        except Exception as e:
