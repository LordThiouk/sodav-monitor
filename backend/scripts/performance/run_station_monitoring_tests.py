    ...
    for line in lines:
        ...
        if station_monitoring_section:
            if line.startswith("Stations: "):
                ...
                results["station_monitoring"][current_station_count]["processing_time"] = processing_time
                ...
                results["station_monitoring"][current_station_count]["stations_per_second"] = stations_per_second
            elif line.startswith("Average Processing Time: "):
                avg_processing_time = float(line.split("Average Processing Time: ")[1].split(" seconds")[0])
                ...
                estimated_optimal_stations = int(line.split("Estimated Optimal Number of Stations for 60-second processing: ")[1])
        ...
        if concurrent_processing_section:
            if line.startswith("Concurrency Level: "):
                ...
                processing_time = float(line.split("Processing Time: ")[1].split(" seconds")[0])
                results["concurrent_processing"][current_concurrency_level]["processing_time"] = processing_time
            elif line.startswith("Stations Per Second: ") and current_concurrency_level is not None:
                stations_per_second = float(line.split("Stations Per Second: ")[1])
                results["concurrent_processing"][current_concurrency_level]["stations_per_second"] = stations_per_second
        ...
        if resource_usage_section:
            ...
            elif line.startswith("Estimated Maximum Number of Stations Based on Available Resources: "):
                max_stations = line.split("Estimated Maximum Number of Stations Based on Available Resources: ")[1]
                try:
...
def generate_plots(results, output_dir):
    ...
    if "station_monitoring" in results:
        ...
        processing_times = [results["station_monitoring"][count]["processing_time"] for count in station_counts]
        stations_per_second = [results["station_monitoring"][count]["stations_per_second"] for count in station_counts]
    ...
    if "concurrent_processing" in results:
        ...
        processing_times = [results["concurrent_processing"][level]["processing_time"] for level in concurrency_levels]
        stations_per_second = [results["concurrent_processing"][level]["stations_per_second"] for level in concurrency_levels]
    ...
    if "resource_usage" in results:
        ...
        memory_usage = [results["resource_usage"][count]["memory_usage"] for count in station_counts]
...
def generate_report(results, output_dir):
    ...
    with open(report_file, 'w') as f:
        ...
        if "estimated_optimal_stations" in results:
            f.write(f"<p>Estimated Optimal Number of Stations for 60-second processing: {results['estimated_optimal_stations']}</p>")
        ...
        if "max_stations" in results:
            f.write(f"<p>Estimated Maximum Number of Stations Based on Available Resources: {results['max_stations']}</p>")
        ...
        if "estimated_optimal_stations" in results:
            f.write(f"<li>For optimal performance, monitor no more than {results['estimated_optimal_stations']} stations simultaneously.</li>")
        if "optimal_concurrency" in results:
            f.write(f"<li>Use a concurrency level of {results['optimal_concurrency']} for best throughput.</li>")
        if "max_stations" in results:
            f.write(f"<li>Based on system resources, the maximum number of stations that can be monitored is approximately {results['max_stations']}.</li>")
