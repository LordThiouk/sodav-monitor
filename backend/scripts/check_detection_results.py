...
def check_detection_results(hours=1):
    ...
    try:
        ...
        detections = db_session.query(Detection).filter(
            Detection.created_at >= time_threshold
        ...
        for detection in detections:
            station = db_session.query(RadioStation).filter(RadioStation.id == detection.station_id).first()
            if not station:
                continue
            ...
            if detections:
                ...
                for detection in detections[:3]:  # Show the 3 most recent
                    ...
                    if detection_type == "music" and detection["track"]:
                        ...
                        print(f"    [{timestamp}] MUSIC: {track_info['title']} by {track_info['artist']}")
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hours", type=int, default=24, help="Number of hours to look back for detections")
