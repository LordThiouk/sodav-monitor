...
from backend.models.models import Track, TrackDetection, Artist, RadioStation
...
def check_recent_detections(hours=24):
    ...
    try:
        ...
        try:
            ...
            log_with_category(logger, "GENERAL", "info", f"Database summary: {track_count} tracks, {artist_count} artists, {detection_count} detections, {station_count} stations")
            
            # Récupérer les détections récentes
            since = datetime.utcnow() - timedelta(hours=hours)
            recent_detections = session.query(TrackDetection).filter(TrackDetection.detected_at >= since).order_by(TrackDetection.detected_at.desc()).all()
            
            log_with_category(logger, "GENERAL", "info", f"Found {len(recent_detections)} detections in the last {hours} hours")
            ...
            if recent_detections:
                ...
                for i, detection in enumerate(recent_detections, 1):
                    ...
                    station = session.query(RadioStation).filter(RadioStation.id == detection.station_id).first()
                    
                    artist_name = artist.name if artist else "Unknown Artist"
                    track_title = track.title if track else "Unknown Track"
                    station_name = station.name if station else "Unknown Station"
                    
                    log_with_category(logger, "GENERAL", "info", f"{i}. {artist_name} - {track_title} on {station_name}")
                    log_with_category(logger, "GENERAL", "info", f"   Detected at: {detection.detected_at}")
                    log_with_category(logger, "GENERAL", "info", f"   Play duration: {detection.play_duration}")
                    log_with_category(logger, "GENERAL", "info", f"   Confidence: {detection.confidence}")
                    log_with_category(logger, "GENERAL", "info", f"   Detection method: {detection.detection_method}")
                    log_with_category(logger, "GENERAL", "info", f"   Fingerprint: {detection.fingerprint[:20]}..." if detection.fingerprint else "   Fingerprint: None")
            else:
            ...
            for i, track in enumerate(recent_tracks, 1):
                artist = session.query(Artist).filter(Artist.id == track.artist_id).first() if track.artist_id else None
                artist_name = artist.name if artist else "Unknown Artist"
                ...
                log_with_category(logger, "GENERAL", "info", f"   Release date: {track.release_date}")
                log_with_category(logger, "GENERAL", "info", f"   Fingerprint: {track.fingerprint[:20]}..." if track.fingerprint else "   Fingerprint: None")
