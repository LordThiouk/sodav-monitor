...
from backend.models.models import Base, StationTrackStats, AnalyticsData
...
def update_schema():
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(text("SELECT column_name FROM information_schema.columns WHERE table_name = 'station_track_stats'"))
        ...
        if 'total_play_time' not in columns:
            ...
            conn.execute(text("ALTER TABLE station_track_stats ADD COLUMN total_play_time INTERVAL DEFAULT '0 seconds'"))
        ...
        if 'average_confidence' not in columns:
            ...
            conn.execute(text("ALTER TABLE station_track_stats ADD COLUMN average_confidence FLOAT DEFAULT 0.0"))
        
        # Vérifier les colonnes de AnalyticsData
        result = conn.execute(text("SELECT column_name FROM information_schema.columns WHERE table_name = 'analytics_data'"))
        columns = [row[0] for row in result]
        ...
        if 'total_tracks' not in columns:
            ...
            conn.execute(text("ALTER TABLE analytics_data ADD COLUMN total_tracks INTEGER DEFAULT 0"))
        
        if 'total_artists' not in columns:
            print("Ajout de la colonne total_artists à analytics_data...")
            conn.execute(text("ALTER TABLE analytics_data ADD COLUMN total_artists INTEGER DEFAULT 0"))
        
        if 'total_stations' not in columns:
            print("Ajout de la colonne total_stations à analytics_data...")
            conn.execute(text("ALTER TABLE analytics_data ADD COLUMN total_stations INTEGER DEFAULT 0"))
