...
def select_primary_track(tracks: List[Track]) -> Track:
    ...
    logger.info(
        f"Piste principale sélectionnée: ID={primary_track.id}, Titre={primary_track.title}, Score={track_scores[0][1]}"
    )

    return primary_track


def merge_track_detections(
    db_session: Session, primary_track_id: int, secondary_track_ids: List[int]
) -> int:
    """
    ...
    try:
        ...
        stmt = (
            update(TrackDetection)
            .where(TrackDetection.track_id.in_(secondary_track_ids))
...
def merge_track_stats(
    ...
    try:
        ...
        primary_stats = (
            db_session.query(TrackStats).filter(TrackStats.track_id == primary_track_id).first()
        )
        ...
        secondary_stats = (
            db_session.query(TrackStats).filter(TrackStats.track_id.in_(secondary_track_ids)).all()
        )
        ...
        for stats in secondary_stats:
            ...
            if stats.last_detected and (
                not primary_stats.last_detected or stats.last_detected > primary_stats.last_detected
            ):
...
def merge_station_track_stats(
    ...
    try:
        ...
        secondary_stats = (
            ...
            .filter(StationTrackStats.track_id.in_(secondary_track_ids))
            .all()
        )
        ...
        for stats in secondary_stats:
            ...
            if primary_stats:
                ...
                if stats.last_played and (
                    not primary_stats.last_played or stats.last_played > primary_stats.last_played
                ):
...
def merge_fingerprints(
    ...
    try:
        ...
        secondary_fingerprints = (
            db_session.query(Fingerprint)
            .filter(Fingerprint.track_id.in_(secondary_track_ids))
            .all()
        )
        ...
        primary_hashes = {
            fp.hash
            for fp in db_session.query(Fingerprint.hash)
            .filter(Fingerprint.track_id == primary_track_id)
            .all()
        }
        ...
        db_session.query(Fingerprint).filter(Fingerprint.track_id.in_(secondary_track_ids)).delete()
    ...
    for isrc, tracks in duplicate_isrc.items():
        ...
        try:
            ...
            if not dry_run:
                ...
                merged_detections = merge_track_detections(
                    db_session, primary_track.id, secondary_track_ids
                )
                ...
                merged_station_stats = merge_station_track_stats(
                    db_session, primary_track.id, secondary_track_ids
                )
                ...
                merged_fingerprints = merge_fingerprints(
                    db_session, primary_track.id, secondary_track_ids
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Recherche et fusionne les pistes dupliquées basées sur l'ISRC."
    )
    parser.add_argument(
