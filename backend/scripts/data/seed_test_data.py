...
async def seed_test_data():
    ...
    try:
        ...
        for i in range(1, 4):
            ...
            existing_station = db.query(RadioStation).filter(RadioStation.name == station_name).first()
            if existing_station:
        ...
        for i in range(1, 11):
            ...
            existing_track = db.query(Track).filter(Track.title == track_title, Track.artist_id == artist.id).first()
            if existing_track:
        ...
        for i in range(50):
            ...
            existing_detection = db.query(TrackDetection).filter(
        ...
        for track in tracks:
            ...
            existing_track_stats = db.query(TrackStats).filter(TrackStats.track_id == track.id).first()
            ...
            for station in stations:
                ...
                existing_station_track_stats = db.query(StationTrackStats).filter(
        ...
        for artist in artists:
            ...
            existing_artist_stats = db.query(ArtistStats).filter(ArtistStats.artist_id == artist.id).first()
        ...
        for station in stations:
            ...
            existing_station_stats = db.query(StationStats).filter(StationStats.station_id == station.id).first()
