    ...
    for track in tracks:
        ...
        if is_valid:
            ...
            if original_isrc != normalized_isrc:
                stats["normalized"] += 1
                logger.info(f"Piste {track.id} ({track.title}): ISRC normalisé de {original_isrc} à {normalized_isrc}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Valide et normalise les codes ISRC dans la base de données.")
