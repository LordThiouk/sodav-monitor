...
from backend.models.models import Artist, Track, TrackStats
...
def create_track(db_session: Session, title: str, artist_id: int, isrc: str = None) -> int:
    ...
    existing_track = (
        db_session.query(Track).filter(Track.title == title, Track.artist_id == artist_id).first()
    )
...
def create_test_data(db_session: Session) -> Dict[str, int]:
    ...
    """
    stats = {"artists": 0, "tracks": 0, "valid_isrc": 0, "invalid_isrc": 0, "duplicate_isrc": 0}
    ...
    try:
        create_track(
            db_session, "Test Track 7", artist3_id, "FR1234567890"
        )  # Dupliqué avec Track 1
        db_session.commit()
        stats["tracks"] += 1
        stats["duplicate_isrc"] += 1
    except Exception as e:
        logger.warning(
            f"Erreur attendue lors de la création d'une piste avec ISRC dupliqué (FR1234567890): {e}"
        )
        db_session.rollback()

    try:
        create_track(
            db_session, "Test Track 8", artist3_id, "US1234567890"
        )  # Dupliqué avec Track 2
        db_session.commit()
        stats["tracks"] += 1
        stats["duplicate_isrc"] += 1
    except Exception as e:
        logger.warning(
            f"Erreur attendue lors de la création d'une piste avec ISRC dupliqué (US1234567890): {e}"
        )
        db_session.rollback()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Crée des pistes de test avec des codes ISRC dans la base de données."
    )
