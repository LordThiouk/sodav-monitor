...
async def test_fingerprint_reuse():
    ...
    try:
        ...
        log_with_category(logger, "GENERAL", "info", "Step 1: Creating test track with unique fingerprint")
        ...
        db.add(track)
        db.commit()
        log_with_category(logger, "GENERAL", "info", f"Created test track: {track_title} with fingerprint: {unique_fingerprint[:20]}...")
        
        # Étape 2 : Simuler une détection avec la même empreinte digitale
        log_with_category(logger, "GENERAL", "info", "Step 2: Simulating detection with the same fingerprint")
        ...
        if detection_result and detection_result.get("source") == "local":
            ...
            log_with_category(logger, "GENERAL", "info", f"Detected track: {detection_result.get('track', {}).get('title')}")
            log_with_category(logger, "GENERAL", "info", f"Source: {detection_result.get('source')}")
            log_with_category(logger, "GENERAL", "info", f"Confidence: {detection_result.get('confidence')}")
            return True
        else:
            log_with_category(logger, "GENERAL", "warning", "Fingerprint reuse test failed")
            log_with_category(logger, "GENERAL", "warning", f"Detection source: {detection_result.get('source') if detection_result else 'None'}")
            return False
