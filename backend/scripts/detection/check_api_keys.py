...
def load_env_file(env_path=None):
    ...
    log_with_category(
        logger, "DETECTION", "info", f"Variables d'environnement chargées depuis {env_path}"
    )
    return True
...
async def check_acoustid_api_key():
    ...
    if not api_key:
        log_with_category(
            ...
            "DETECTION",
            "error",
            "Clé API AcoustID non définie dans les variables d'environnement",
        )
        return False

    log_with_category(
        logger, "DETECTION", "info", f"Clé API AcoustID trouvée: {api_key[:5]}...{api_key[-3:]}"
    )
    ...
    if not fpcalc_path:
        log_with_category(
            logger, "DETECTION", "error", "fpcalc non trouvé, nécessaire pour AcoustID"
        )
        return False
    ...
    try:
        ...
        if result:
            log_with_category(
                logger, "DETECTION", "info", "Test AcoustID réussi avec recherche par métadonnées"
            )
            log_with_category(logger, "DETECTION", "info", f"Résultat: {json.dumps(result)}")
            return True
        else:
            log_with_category(
                logger, "DETECTION", "warning", "Test AcoustID échoué: aucun résultat trouvé"
            )
            return False
...
async def check_audd_api_key():
    ...
    if not api_key:
        log_with_category(
            ...
            "DETECTION",
            "error",
            "Clé API AudD non définie dans les variables d'environnement",
        )
        return False

    log_with_category(
        logger, "DETECTION", "info", f"Clé API AudD trouvée: {api_key[:5]}...{api_key[-3:]}"
    )
    ...
    try:
        ...
        log_with_category(
            ...
            "DETECTION",
            "info",
            "Ce test peut échouer si l'URL n'est pas accessible, mais vérifie quand même la validité de la clé API",
        )
        ...
        if result:
            ...
            log_with_category(
                ...
                "DETECTION",
                "warning",
                "Test AudD échoué: aucun résultat trouvé (peut être normal si l'URL de test n'est pas valide)",
            )
...
async def check_musicbrainz_setup():
    ...
    try:
        ...
        if result:
            ...
            log_with_category(
                logger, "DETECTION", "warning", "Test MusicBrainz échoué: aucun résultat trouvé"
            )
            return False
    except Exception as e:
...
async def main():
    """Fonction principale"""
    log_with_category(
        logger, "DETECTION", "info", "Vérification des clés API des services externes"
    )
    ...
    log_with_category(
        logger, "DETECTION", "info", f"MusicBrainz: {'OK' if musicbrainz_ok else 'ERREUR'}"
    )
