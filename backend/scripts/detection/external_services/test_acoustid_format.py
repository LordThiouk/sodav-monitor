...
def generate_fingerprint(audio_file_path):
    ...
    try:
        ...
        if not fpcalc_path:
            logger.error(
                "fpcalc not found. Please make sure it's installed and available in the PATH or in the project's bin directory."
            )
            return None, None
        ...
        result = subprocess.run(
            [fpcalc_path, "-json", str(audio_file_path)], capture_output=True, text=True
        )
