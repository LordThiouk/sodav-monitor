    ...
    for track in test_tracks:
        ...
        try:
            ...
            if result:
                ...
                log_with_category(logger, "TEST", "warning", f"No result found for: {title} by {artist}")
                
        except Exception as e:
            log_with_category(logger, "TEST", "error", f"Error searching for track: {str(e)}")
    
    log_with_category(logger, "TEST", "info", f"Test completed: {success_count}/{len(test_tracks)} tracks successfully found")
...
async def main():
    ...
    if result:
        log_with_category(logger, "TEST", "info", "MusicBrainz metadata search test passed successfully")
    else:
