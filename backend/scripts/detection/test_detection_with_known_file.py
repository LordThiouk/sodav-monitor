...
async def test_detection_with_known_file():
    ...
    try:
        ...
        if detection_result and detection_result.get("type") == "music":
            ...
            log_with_category(logger, "GENERAL", "info", f"Track: {track_info.get('title', 'Unknown')}")
            log_with_category(logger, "GENERAL", "info", f"Artist: {track_info.get('artist', 'Unknown')}")
            ...
            log_with_category(logger, "GENERAL", "info", f"Detections in database: {detection_count}")
            ...
            log_with_category(logger, "GENERAL", "info", f"After explicit commit - Tracks: {new_track_count}, Detections: {new_detection_count}")
            ...
            if new_track_count > 0 and new_detection_count > 0:
                ...
                log_with_category(logger, "GENERAL", "warning", "Detection test failed: No tracks or detections found in database")
                return False
        else:
