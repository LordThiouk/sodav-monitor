...
async def test_audd_detection():
    ...
    if not audd_api_key:
        log_with_category(logger, "GENERAL", "error", "AUDD_API_KEY not found in environment variables")
        return False
    ...
    try:
        ...
        log_with_category(logger, "GENERAL", "info", f"Calling AudD detection directly with station_id={station_id}...")
        ...
        if audd_result:
            ...
            log_with_category(logger, "GENERAL", "info", f"Track: {track_info.get('title')} by {track_info.get('artist')}")
            log_with_category(logger, "GENERAL", "info", f"Album: {track_info.get('album')}")
            log_with_category(logger, "GENERAL", "info", f"Confidence: {audd_result.get('confidence')}")
            ...
            log_with_category(logger, "GENERAL", "info", f"Release date: {track_info.get('release_date')}")
            log_with_category(logger, "GENERAL", "info", f"Fingerprint: {track_info.get('fingerprint')}")
            log_with_category(logger, "GENERAL", "info", f"Play duration: {audd_result.get('play_duration')} seconds")
            ...
            if station_id:
                ...
                detection = db.query(TrackDetection).filter_by(station_id=station_id).order_by(TrackDetection.detected_at.desc()).first()
                
                if detection:
                    log_with_category(logger, "GENERAL", "info", f"Play time recorded in database: {detection.play_duration}")
                    ...
                    log_with_category(logger, "GENERAL", "warning", "No detection record found in database")
...
async def main():
    ...
    if result:
        log_with_category(logger, "GENERAL", "info", "Enhanced AudD detection test passed successfully")
    else:
