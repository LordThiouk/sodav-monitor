...
async def test_detection_hierarchy():
    ...
    try:
        ...
        if not test_file_path.exists():
            log_with_category(
                logger, "TEST", "error", f"Test audio file not found at {test_file_path}"
            )
            return False
        ...
        try:
            ...
            metadata = {"artist": "Michael Jackson", "title": "Thriller"}
        ...
        if result.get("type") == "music":
            log_with_category(
                logger, "TEST", "info", f"Music detected with source: {result.get('source')}"
            )
