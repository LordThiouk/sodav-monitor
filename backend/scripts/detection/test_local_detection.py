    ...
    try:
        ...
        if existing_track:
            log_with_category(
                ...
                "GENERAL",
                "info",
                f"Found existing track with fingerprint: {fingerprint[:20]}...",
            )
            ...
            if not track:
                ...
                try:
                    ...
                    log_with_category(
                        ...
                        "GENERAL",
                        "info",
                        f"Created test track: {track_title} with fingerprint: {fingerprint[:20]}...",
                    )
                except Exception as e:
                    db.rollback()
                    log_with_category(
                        logger, "GENERAL", "warning", f"Error creating track: {str(e)}"
                    )
                    ...
                    if track:
                        log_with_category(
                            ...
                            "GENERAL",
                            "info",
                            f"Using existing track with fingerprint: {fingerprint[:20]}...",
                        )
                    else:
                        log_with_category(
                            logger,
                            "GENERAL",
                            "error",
                            "Could not find or create track with fingerprint",
                        )
                        return False
            else:
                ...
                try:
                    ...
                    log_with_category(
                        ...
                        "GENERAL",
                        "info",
                        f"Updated test track: {track_title} with fingerprint: {fingerprint[:20]}...",
                    )
                except Exception as e:
                    db.rollback()
                    log_with_category(
                        logger, "GENERAL", "warning", f"Error updating track: {str(e)}"
                    )
                    # Essayer de récupérer la piste existante avec cette empreinte
                    track = db.query(Track).filter(Track.fingerprint == fingerprint).first()
                    if track:
                        ...
                        log_with_category(
                            ...
                            "GENERAL",
                            "error",
                            "Could not find or update track with fingerprint",
                        )
                        return False
        ...
        if local_result:
            ...
            log_with_category(
                ...
                "GENERAL",
                "info",
                f"Detected track: {local_result.get('title')} by {local_result.get('artist')}",
            )
            log_with_category(
                logger, "GENERAL", "info", f"Confidence: {local_result.get('confidence')}"
            )
            log_with_category(
                logger, "GENERAL", "info", f"Fingerprint: {local_result.get('fingerprint')[:20]}..."
            )
            return True
        else:
