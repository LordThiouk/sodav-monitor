...
def verify_migrations():
    ...
    try:
        ...
        log_with_category(
            logger, "VERIFICATION", "info", f"Colonnes de la table tracks: {track_columns}"
        )
        ...
        if "release_date" in track_columns:
            log_with_category(
                ...
                "VERIFICATION",
                "info",
                "La colonne release_date existe dans la table tracks ✅",
            )
        else:
            log_with_category(
                logger,
                "VERIFICATION",
                "error",
                "La colonne release_date n'existe pas dans la table tracks ❌",
            )

        if "genre" in track_columns:
            log_with_category(
                logger, "VERIFICATION", "info", "La colonne genre existe dans la table tracks ✅"
            )
        else:
            log_with_category(
                logger,
                "VERIFICATION",
                "error",
                "La colonne genre n'existe pas dans la table tracks ❌",
            )
        ...
        log_with_category(
            ...
            "VERIFICATION",
            "info",
            f"Colonnes de la table track_detections: {detection_columns}",
        )
        ...
        if "detection_method" in detection_columns:
            log_with_category(
                ...
                "VERIFICATION",
                "info",
                "La colonne detection_method existe dans la table track_detections ✅",
            )
        else:
            log_with_category(
                logger,
                "VERIFICATION",
                "error",
                "La colonne detection_method n'existe pas dans la table track_detections ❌",
            )

        # Vérifier si les tables existent
        tables = inspector.get_table_names()
        log_with_category(
            logger, "VERIFICATION", "info", f"Tables dans la base de données: {tables}"
        )

        return True
    except Exception as e:
        log_with_category(
            logger, "VERIFICATION", "error", f"Erreur lors de la vérification: {str(e)}"
        )
        return False
    finally:
        session.close()
