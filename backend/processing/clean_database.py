...
from models import Artist, RadioStation, Track, TrackDetection
...
def clean_database():
    ...
    try:
        ...
        track_ids = [
            t.id
            for t in session.query(Track.id).filter(Track.title.in_(non_senegalese_tracks)).all()
        ]
        ...
        artist_track_ids = [
            t.id
            for t in session.query(Track.id)
            .join(Artist)
            .filter(Artist.name.in_(non_senegalese_artists))
            .all()
        ]
        ...
        if artist_track_ids:
            ...
            detections_deleted = (
                session.query(TrackDetection)
                .filter(TrackDetection.track_id.in_(artist_track_ids))
                .delete(synchronize_session=False)
            )
            logger.info(
                f"Deleted {detections_deleted} detections for tracks of non-Senegalese artists"
            )

            # Delete the tracks
            tracks_deleted = (
                session.query(Track)
                .filter(Track.id.in_(artist_track_ids))
                .delete(synchronize_session=False)
            )
        ...
        duplicate_tracks = (
            session.query(Track.title, Artist.id, func.count(Track.id).label("count"))
        ...
        for title, artist_id, count in duplicate_tracks:
            ...
            if len(tracks) > 1:
                ...
                detections_deleted = (
                    session.query(TrackDetection)
                    .filter(TrackDetection.track_id.in_(track_ids_to_delete))
                    .delete(synchronize_session=False)
                )
                ...
                tracks_deleted = (
                    session.query(Track)
                    .filter(Track.id.in_(track_ids_to_delete))
                    .delete(synchronize_session=False)
                )
