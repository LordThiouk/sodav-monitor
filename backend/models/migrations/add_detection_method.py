...
def add_detection_method():
    ...
    try:
        ...
        if 'detection_method' not in columns:
            log_with_category(logger, "MIGRATION", "info", "Ajout de la colonne detection_method à la table track_detections")
            session.execute(text('ALTER TABLE track_detections ADD COLUMN detection_method VARCHAR'))
        else:
            log_with_category(logger, "MIGRATION", "info", "La colonne detection_method existe déjà dans la table track_detections")
...
if __name__ == "__main__":
    log_with_category(logger, "MIGRATION", "info", "Démarrage de la migration pour ajouter le champ detection_method à la table track_detections")
