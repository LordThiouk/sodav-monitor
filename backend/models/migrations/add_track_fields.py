...
def add_track_fields():
    ...
    try:
        ...
        if "release_date" not in columns:
            log_with_category(
                logger, "MIGRATION", "info", "Ajout de la colonne release_date à la table tracks"
            )
            ...
            log_with_category(
                ...
                "MIGRATION",
                "info",
                "La colonne release_date existe déjà dans la table tracks",
            )

        # Ajouter la colonne genre si elle n'existe pas
        if "genre" not in columns:
            log_with_category(
                logger, "MIGRATION", "info", "Ajout de la colonne genre à la table tracks"
            )
            session.execute(text("ALTER TABLE tracks ADD COLUMN genre VARCHAR"))
        else:
            log_with_category(
                logger, "MIGRATION", "info", "La colonne genre existe déjà dans la table tracks"
            )
...
if __name__ == "__main__":
    log_with_category(
        ...
        "MIGRATION",
        "info",
        "Démarrage de la migration pour ajouter les champs release_date et genre au modèle Track",
    )
