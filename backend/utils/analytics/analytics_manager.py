"""Module for managing analytics data in the SODAV Monitor system.

This module provides functionality for updating and managing analytics data.
"""
import logging
...
class AnalyticsManager:
    """Class for managing analytics data in the SODAV Monitor system."""

    def __init__(self, db_session: Session):
        """Initialize the AnalyticsManager with a database session.
        ...
        try:
            ...
            async with self.db.begin():
                ...
                if not isinstance(confidence, (int, float)) or confidence < 0 or confidence > 1:
                    raise ValueError(
                        f"confidence must be a float between 0 and 1, got {confidence}"
                    )
                if not isinstance(play_duration, timedelta):
                    raise ValueError(
                        f"play_duration must be a timedelta, got {type(play_duration)}"
                    )

                # Use a single query to get all required data
                base_data = await self.db.execute(
                    text(
                        """
                    SELECT
                        t.id as track_id,
                ...
                """
                    ...
                    {"station_id": station_id, "track_id": track_id},
                )
                ...
                await self._update_detection_stats_efficient(
                    station_id, track_id, confidence, play_duration, current_time, base_data
                )
    ...
    async def _update_detection_stats_efficient(
        ...
        Args:
            ...
            track_id: ID of the detected track
            confidence: Detection confidence score
            play_duration: Duration of the track play
            current_time: Current timestamp
            base_data: Base data for the detection
        """
        try:
            ...
            if base_data and base_data["artist_id"]:
                ...
                artist_update_query = """
                    ...
                    SET
                        ...
                        total_play_time = COALESCE(
                            total_play_time, '0 seconds'::interval
                        ) + :play_duration,
                        updated_at = :current_time
                    WHERE id = :artist_id
                """
                await self.db.execute(
                    text(artist_update_query),
                    {
                ...
                artist_stats_query = """
                    ...
                    ON CONFLICT (artist_id) DO UPDATE SET
                        ...
                        last_detected = :current_time,
                        total_play_time = COALESCE(
                            artist_stats.total_play_time, '0 seconds'::interval
                        ) + :play_duration,
                        average_confidence = (
            ...
            station_track_query = """
                ...
                ON CONFLICT (station_id, track_id) DO UPDATE SET
                    ...
                    total_play_time = COALESCE(
                        station_track_stats.total_play_time, '0 seconds'::interval
                    ) + :play_duration,
                    last_played = :current_time,
                    average_confidence = (
                        station_track_stats.average_confidence *
                        station_track_stats.play_count + :confidence
                    ) / (station_track_stats.play_count + 1)
            """
            await self.db.execute(
                text(station_track_query),
                {
                    "station_id": station_id,
                    "track_id": track_id,
                    "play_duration": play_duration,
    ...
    async def _update_analytics_data_efficient(
        self, confidence: float, current_time: datetime
    ) -> None:
        ...
        try:
            ...
            """
                ...
                {"hour_ago": current_time - timedelta(hours=1), "current_time": current_time},
            )
        ...
        try:
            ...
            temporal_query = """
                ...
                ON CONFLICT (artist_id, date) DO UPDATE SET
                    ...
                    total_play_time = COALESCE(
                        artist_daily.total_play_time, '0 seconds'::interval
                    ) + :play_duration;
                ...
                ON CONFLICT (artist_id, month) DO UPDATE SET
                    ...
                    total_play_time = COALESCE(
                        artist_monthly.total_play_time, '0 seconds'::interval
                    ) + :play_duration;
            """
            await self.db.execute(
                text(temporal_query),
    ...
    async def _update_station_status_efficient(
        self, station_id: int, current_time: datetime
    ) -> None:
        ...
        try:
            ...
            """
                ...
                {"station_id": station_id, "current_time": current_time, "hour_ago": hour_ago},
            )
