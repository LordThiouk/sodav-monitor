            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Error fetching stations for country {country}: {response.status}"
                        )
                        return []
...
async def fetch_and_save_senegal_stations(db: Session) -> int:
    ...
    try:
        ...
        if not inspector.has_table("radio_stations"):
            logger.warning(
                "radio_stations table does not exist yet. Will be created during next database initialization."
            )
            return 0
        ...
        for station_data in stations:
            try:
                ...
                existing_station = (
                    ...
                    .filter(RadioStation.stream_url == station_data["url_resolved"])
                    .first()
                )
                ...
                logger.error(
                    f"Error processing station {station_data.get('name', 'unknown')}: {str(station_error)}"
                )
                continue
        ...
        db.commit()
        logger.info(
            f"Successfully processed {stations_added + stations_updated} Senegalese stations (Added: {stations_added}, Updated: {stations_updated})"
        )
