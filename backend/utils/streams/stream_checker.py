...
class StreamChecker:
    ...
    async def check_stream_availability(
        self, stream_url: str, timeout: Optional[int] = None
    ) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            return {"available": False, "accessible": False, "valid_format": False}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(stream_url, timeout=timeout) as response:
                    ...
                    if any([icy_name, icy_genre, icy_br]):
                        return {"name": icy_name, "genre": icy_genre, "bitrate": icy_br}
                    return None
