...
from ..models.models import (
    ...
    StationTrackStats, AnalyticsData, ArtistStats, TrackStats, StationStats
)
import logging
...
class StatsManager:
    ...
    def update_stats(self):
        ...
        try:
            ...
            detections = self.db_session.query(TrackDetection).filter(
                TrackDetection.detected_at >= datetime.utcnow() - timedelta(hours=24)
            ).order_by(TrackDetection.detected_at).all()
            ...
            for artist_id in artist_ids:
                ...
                artist_detections = [d for d in detections if d.track and d.track.artist_id == artist_id]
                ...
                for detection in artist_detections:
                    ...
                    if not artist_stats.last_detected or detection.detected_at > artist_stats.last_detected:
    ...
    def update_analytics_data(self):
        ...
        try:
            ...
            detections_24h = self.db_session.query(func.count(TrackDetection.id)).filter(
                TrackDetection.detected_at >= now - timedelta(hours=24)
            ).scalar() or 0
        ...
        try:
            ...
            if detection.track.artist_id:
                ...
                artist_stats.average_confidence = (
                    (artist_stats.average_confidence * (artist_stats.total_plays - 1) + detection.confidence)
                    / artist_stats.total_plays
                ) if artist_stats.total_plays > 0 else detection.confidence
    ...
    def update_track_stats(self, track_id: int) -> None:
        ...
        try:
            ...
            for detection in detections:
                ...
                track_stats.total_play_time += detection.play_duration
                total_confidence += detection.confidence
                if not track_stats.last_detected or detection.detected_at > track_stats.last_detected:
                    track_stats.last_detected = detection.detected_at
    ...
    def update_station_stats(self, station_id: int) -> None:
        ...
        try:
            ...
            station_stats = self.db_session.query(StationStats).filter_by(station_id=station_id).first()
            ...
            for detection in detections:
                ...
                total_confidence += detection.confidence
                if not station_stats.last_detected or detection.detected_at > station_stats.last_detected:
    ...
    def update_station_track_stats(self, station_id: int, track_id: int) -> None:
        ...
        try:
            ...
            if not detections:
                logger.info(f"No recent detections found for station {station_id} and track {track_id}")
                return
            ...
            for detection in detections:
                ...
                if detection_time.tzinfo is not None or (last_played_time is not None and last_played_time.tzinfo is not None):
            ...
            if station_track_stats.play_count > 0:
                station_track_stats.average_confidence = total_confidence / station_track_stats.play_count
    ...
    def update_all_stats(self) -> None:
        ...
        try:
            ...
            detections = self.db_session.query(TrackDetection).filter(
                TrackDetection.detected_at >= one_day_ago
            ).order_by(TrackDetection.detected_at).all()
            ...
            for station_id in station_ids:
                for track_id in track_ids:
                    ...
                    detection_exists = any(
                        d.station_id == station_id and d.track_id == track_id
                        for d in detections
                    )
                    if detection_exists:
