...
class StatisticsInitializer:
    ...
    def _initialize_track_stats(self):
        ...
        for track in tracks:
            ...
            stats = (
                ...
                .filter(TrackDetection.track_id == track.id)
                .first()
            )
    ...
    def _initialize_station_track_stats(self):
        ...
        for station in stations:
            ...
            detections = (
                ...
                .group_by(TrackDetection.track_id)
                .all()
            )
    ...
    def _initialize_time_based_stats(self):
        ...
        for date in dates:
            # Hourly stats
            hourly = DetectionHourly(hour=date.hour, count=date.count)
            self.session.add(hourly)

            # Daily stats
            daily = DetectionDaily(date=date.day, count=date.count)
            self.session.add(daily)

            # Monthly stats
            monthly = DetectionMonthly(month=date.month, count=date.count)
    ...
    def _initialize_analytics_data(self):
        ...
        active_stations = (
            self.session.query(func.count(RadioStation.id))
            .filter(RadioStation.is_active == True)
            .scalar()
        )
        avg_confidence = self.session.query(func.avg(TrackDetection.confidence)).scalar()
        ...
        hourly_detections = (
            self.session.query(func.count(TrackDetection.id))
            .filter(TrackDetection.detected_at >= hour_ago)
            .scalar()
        )
