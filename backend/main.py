...
from backend.core.config import PATHS, get_settings
    ...
    while detection_running:
        try:
            ...
            try:
                ...
                active_stations = (
                    db.query(RadioStation).filter(RadioStation.status == StationStatus.ACTIVE).all()
                )

                if active_stations:
                    logger.info(
                        f"Running automatic detection for {len(active_stations)} active stations"
                    )
                    ...
                    for station in active_stations:
                        ...
                        try:
                            logger.info(
                                f"Detecting music for station: {station.name} (ID: {station.id})"
                            )
                            ...
                            logger.error(
                                f"Error detecting music for station {station.name}: {str(e)}"
                            )
                else:
...
async def root():
    """Root endpoint."""
    return {"message": "Welcome to SODAV Monitor API", "version": "1.0.0", "docs_url": "/docs"}
...
async def get_track_by_isrc(isrc: str, db: Session = Depends(get_db)):
    ...
    if result:
        ...
        return {"success": False, "message": f"Aucune piste trouvée avec l'ISRC {isrc}"}
