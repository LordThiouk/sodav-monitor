...
import pandas as pd
from backend.models.models import TrackDetection, RadioStation
...
class ReportGenerator:
    ...
    async def _generate_detection_report(
        ...
        filename = f"detection_report_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.xlsx"
    ...
    async def _generate_station_report(
        ...
        # Generate report file
        filename = f"station_report_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.xlsx"
        filepath = f"reports/{filename}"
        df.to_excel(filepath, index=False)
