...
class ReportGenerator:
    ...
    async def _generate_pdf_report(
        self, daily_stats: List[Dict], start_date: datetime, end_date: datetime
    ) -> str:
        """
        ...
        try:
            ...
            for stats in daily_stats:
                for track in stats["top_tracks"][:5]:  # Top 5 par jour
                    pdf.cell(
                        0,
                        10,
                        f"{track['title']} - {track['artist']} ({track['detections']} détections)",
                        ln=True,
                    )
            ...
            for stats in daily_stats:
                for artist in stats["top_artists"][:5]:  # Top 5 par jour
                    pdf.cell(
                        0, 10, f"{artist['name']} ({artist['detections']} détections)", ln=True
                    )
            ...
            for stats in daily_stats:
                for station in stats["station_stats"]:
                    pdf.cell(
                        0, 10, f"{station['name']}: {station['detections']} détections", ln=True
                    )
    ...
    async def _generate_excel_report(
        ...
        try:
            ...
            with pd.ExcelWriter(filepath) as writer:
                ...
                pd.DataFrame(stations_data).to_excel(
                    writer, sheet_name="Statistiques Stations", index=False
                )
