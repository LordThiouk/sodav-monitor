...
from backend.utils.logging_config import LOG_CATEGORIES, log_with_category, setup_logging
...
class AcoustIDService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not api_key:
            ...
            try:
                ...
                log_with_category(
                    ...
                    "ACOUSTID",
                    "error",
                    f"Error loading API key from .env.development: {str(e)}",
                )
        ...
        if api_key:
            log_with_category(
                ...
                "ACOUSTID",
                "info",
                f"AcoustID API key loaded: {api_key[:5]}...{api_key[-3:]}",
            )
        else:
        ...
        try:
            ...
            if not fpcalc_path:
                log_with_category(
                    ...
                    "ACOUSTID",
                    "warning",
                    "fpcalc not available, cannot generate fingerprint",
                )
                return None, 0
            ...
            try:
                ...
                log_with_category(
                    ...
                    "ACOUSTID",
                    "info",
                    f"Generating fingerprint with fpcalc for file: {temp_file_path}",
                )
                result = subprocess.run(
                    [fpcalc_path, "-json", temp_file_path], capture_output=True, text=True
                )

                if result.returncode != 0:
                    log_with_category(
                        logger, "ACOUSTID", "error", f"fpcalc failed with error: {result.stderr}"
                    )
                    return None, 0
                ...
                if not fingerprint or not duration:
                    log_with_category(
                        logger, "ACOUSTID", "error", "Failed to generate fingerprint or duration"
                    )
                    return None, 0
                ...
                log_with_category(
                    ...
                    "ACOUSTID",
                    "info",
                    f"Generated fingerprint: {fingerprint[:20]}... (length: {len(fingerprint)})",
                )
            ...
            log_with_category(
                logger, "ACOUSTID", "error", f"Error generating fingerprint: {str(e)}"
            )
    ...
    async def detect_track(self, audio_data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            log_with_category(
                ...
                "ACOUSTID",
                "info",
                f"Sending request to AcoustID: {url} with fingerprint length {len(fingerprint)}",
            )
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    ...
                    log_with_category(
                        logger, "ACOUSTID", "info", "Sending POST request to AcoustID..."
                    )
                    ...
                    async with session.post(url, data=params, timeout=30) as response:
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "info",
                            f"AcoustID response status: {response.status}",
                        )
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "info",
                            f"AcoustID response text (first 500 chars): {response_text[:500]}...",
                        )

                        if response.status != 200:
                            log_with_category(
                                logger, "ACOUSTID", "error", f"AcoustID error: {response_text}"
                            )
                            return None
                        ...
                        try:
                            ...
                            log_with_category(
                                logger, "ACOUSTID", "info", f"Successfully parsed JSON response"
                            )
                            ...
                            log_with_category(
                                ...
                                "ACOUSTID",
                                "error",
                                f"Failed to parse AcoustID response as JSON: {e}",
                            )
                            log_with_category(
                                logger,
                                "ACOUSTID",
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "debug",
                            f"AcoustID complete response: {json.dumps(data)}",
                        )
                        ...
                        if "status" not in data or data["status"] != "ok":
                            log_with_category(
                                ...
                                "ACOUSTID",
                                "error",
                                f"AcoustID returned non-OK status: {data.get('status', 'unknown')}",
                            )
                            if "error" in data:
                                log_with_category(
                                    logger,
                                    "ACOUSTID",
                                    "error",
                                    f"AcoustID error message: {data['error'].get('message', 'No message')}",
                                )
                            return None
                        ...
                        if "results" not in data or not data["results"]:
                            log_with_category(
                                logger, "ACOUSTID", "info", "No AcoustID results found"
                            )
                            return None
                        ...
                        log_with_category(
                            logger, "ACOUSTID", "info", f"AcoustID best result score: {score}"
                        )

                        # Check if recordings are found
                        if "recordings" not in best_result or not best_result["recordings"]:
                            log_with_category(
                                logger, "ACOUSTID", "info", "No recordings found in AcoustID result"
                            )
                            return None
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "info",
                            f"AcoustID detection result: {title} by {artist}, confidence: {score}",
                        )

                        return result
                except aiohttp.ClientError as e:
                    log_with_category(
                        logger, "ACOUSTID", "error", f"AcoustID client error: {str(e)}"
                    )
                    return None

        except asyncio.TimeoutError:
            log_with_category(
                logger, "ACOUSTID", "error", "AcoustID request timed out after 30 seconds"
            )
            return None
        except Exception as e:
            log_with_category(
                logger, "ACOUSTID", "error", f"Error detecting track with AcoustID: {str(e)}"
            )
            log_with_category(logger, "ACOUSTID", "error", f"Traceback: {traceback.format_exc()}")
            return None
    ...
    async def search_by_metadata(self, artist: str, title: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Found recording: {recording.get('title')} by {recording.get('artist-credit-phrase')}"
            )
    ...
    async def detect_track_with_retry(
        ...
        log_with_category(
            ...
            "ACOUSTID",
            "info",
            f"Starting track detection with retry (max_retries={max_retries})",
        )
        ...
        if not api_works:
            log_with_category(
                ...
                "ACOUSTID",
                "error",
                "AcoustID API test failed, cannot proceed with detection",
            )
            return None

        log_with_category(
            logger, "ACOUSTID", "info", "AcoustID API test successful, proceeding with detection"
        )
        ...
        for retry in range(max_retries):
            ...
            try:
                ...
                log_with_category(
                    ...
                    "ACOUSTID",
                    "info",
                    f"Trying with audio segment starting at {skip_seconds} seconds",
                )
                ...
                if audio is None:
                    log_with_category(
                        logger, "ACOUSTID", "error", "Failed to load audio data for retry"
                    )
                    continue
                ...
                if start_frame >= len(audio):
                    log_with_category(
                        ...
                        "ACOUSTID",
                        "warning",
                        f"Audio too short for segment starting at {skip_seconds} seconds",
                    )
                    continue
                ...
                log_with_category(
                    logger, "ACOUSTID", "info", f"Extracted segment of {len(segment_bytes)} bytes"
                )
                ...
                if result:
                    log_with_category(
                        logger, "ACOUSTID", "info", f"Track detected on retry {retry+1}"
                    )
                    return result

            except Exception as e:
                log_with_category(
                    logger, "ACOUSTID", "error", f"Error in retry attempt {retry+1}: {str(e)}"
                )
                import traceback

                log_with_category(
                    logger, "ACOUSTID", "error", f"Traceback: {traceback.format_exc()}"
                )
    ...
    async def detect_track_from_station(
        ...
        try:
            ...
            log_with_category(
                ...
                "ACOUSTID",
                "info",
                f"Detecting track from station {station_name} (ID: {station_id}) at {timestamp}",
            )
            log_with_category(
                logger, "ACOUSTID", "info", f"Audio data size: {len(audio_data)} bytes"
            )
            ...
            if not fingerprint_result:
                log_with_category(
                    ...
                    "ACOUSTID",
                    "error",
                    f"Failed to generate fingerprint for station {station_name}",
                )
                return None
            ...
            log_with_category(
                ...
                "ACOUSTID",
                "info",
                f"Generated fingerprint for station {station_name}: {fingerprint[:20]}... (length: {len(fingerprint)})",
            )
            log_with_category(logger, "ACOUSTID", "info", f"Duration: {duration} seconds")
            ...
            log_with_category(
                ...
                "ACOUSTID",
                "info",
                f"Sending request to AcoustID for station {station_name}",
            )
                ...
                try:
                    ...
                    async with session.post(url, data=params, timeout=30) as response:
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "info",
                            f"AcoustID response status for station {station_name}: {response.status}",
                        )
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "info",
                            f"AcoustID response text for station {station_name} (first 500 chars): {response_text[:500]}...",
                        )
                        ...
                        if response.status != 200:
                            log_with_category(
                                ...
                                "ACOUSTID",
                                "error",
                                f"AcoustID error for station {station_name}: {response_text}",
                            )
                            return None
                        ...
                        try:
                            ...
                            log_with_category(
                                ...
                                "ACOUSTID",
                                "info",
                                f"Successfully parsed JSON response for station {station_name}",
                            )
                        except json.JSONDecodeError as e:
                            log_with_category(
                                logger,
                                "ACOUSTID",
                                "error",
                                f"Failed to parse AcoustID response as JSON for station {station_name}: {e}",
                            )
                            return None
                        ...
                        if "status" not in data or data["status"] != "ok":
                            log_with_category(
                                ...
                                "ACOUSTID",
                                "error",
                                f"AcoustID returned non-OK status for station {station_name}: {data.get('status', 'unknown')}",
                            )
                            return None
                        ...
                        if "results" not in data or not data["results"]:
                            log_with_category(
                                ...
                                "ACOUSTID",
                                "info",
                                f"No AcoustID results found for station {station_name}",
                            )
                            return None
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "info",
                            f"AcoustID best result score for station {station_name}: {score}",
                        )
                        ...
                        if "recordings" not in best_result or not best_result["recordings"]:
                            log_with_category(
                                ...
                                "ACOUSTID",
                                "info",
                                f"No recordings found in AcoustID result for station {station_name}",
                            )
                            return None
                        ...
                        log_with_category(
                            ...
                            "ACOUSTID",
                            "info",
                            f"AcoustID detection result for station {station_name}: {title} by {artist}, confidence: {score}",
                        )
                    ...
                    log_with_category(
                        ...
                        "ACOUSTID",
                        "error",
                        f"AcoustID client error for station {station_name}: {str(e)}",
                    )
                    return None

        except Exception as e:
            log_with_category(
                logger, "ACOUSTID", "error", f"Error detecting track from station data: {str(e)}"
            )
            log_with_category(logger, "ACOUSTID", "error", f"Traceback: {traceback.format_exc()}")
            return None
...
class AuddService:
    ...
    async def detect_track(self, audio_data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data = {"api_token": self.api_key, "return": "spotify,apple_music,musicbrainz,deezer"}

            files = {"file": ("audio.mp3", audio_file, "audio/mpeg")}

            log_with_category(
                logger, "AUDD", "info", f"Sending audio data to AudD API: {len(audio_data)} bytes"
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.base_url, data=data, files=files) as response:
                    # Log response status
                    log_with_category(
                        logger, "AUDD", "info", f"AudD response status: {response.status}"
                    )
                    ...
                    # Log complete response for debugging
                    log_with_category(
                        logger, "AUDD", "debug", f"AudD complete response: {json.dumps(result)}"
                    )
                    ...
                    log_with_category(
                        ...
                        "AUDD",
                        "info",
                        f"AudD found track: {track_data.get('title', 'Unknown')} by {track_data.get('artist', 'Unknown')}",
                    )
                    ...
                    if "spotify" in track_data:
                        ...
                        if "album" in spotify_data and "release_date" not in detection_result:
                            detection_result["release_date"] = spotify_data["album"].get(
                                "release_date"
                            )
                    ...
                    log_with_category(
                        ...
                        "AUDD",
                        "info",
                        f"AudD detection result: {detection_result['title']} by {detection_result['artist']}",
                    )
    ...
    async def detect_track_with_url(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            data = {"api_token": self.api_key, "url": url, "return": "apple_music,spotify"}

            logger.info(f"Sending URL request to AudD API: {url}")

            async with aiohttp.ClientSession() as session:
                async with session.post(self.base_url, data=data, timeout=30) as response:
                    if response.status != 200:
    ...
    async def detect_track_with_retry(
        self, audio_data: bytes, max_retries: int = 3, retry_delay: float = 1.0
    ) -> Optional[Dict[str, Any]]:
        """
...
class ExternalServiceHandler:
    ...
    async def recognize_with_acoustid(
        self, audio_data: bytes, max_retries: int = 3
    ) -> Optional[Dict[str, Any]]:
        ...
        while retries <= max_retries:
            try:
                ...
                result = musicbrainzngs.search_recordings(
                    query=f"duration:{int(features['duration'])}", limit=5
                )
    ...
    async def recognize_with_musicbrainz_metadata(
        self, artist: str, title: str, max_retries: int = 3
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    async def recognize_with_audd(
        ...
        """
        try:
            return await self.audd_service.detect_track_with_retry(
                audio_data, max_retries=max_retries
            )
        except Exception as e:
            logger.error(f"Error recognizing with Audd: {str(e)}")
            return None

    async def recognize_with_audd_url(
    ...
    async def recognize_with_acoustid_from_station(
        self, station_data: Dict[str, Any], max_retries: int = 3
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        log_with_category(
            ...
            "GENERAL",
            "info",
            f"Recognizing track from station {station_name} using AcoustID",
        )
        ...
        if not self.acoustid_service:
            log_with_category(
                ...
                "GENERAL",
                "error",
                f"AcoustID service not initialized for station {station_name}",
            )
            return None
        ...
        try:
            ...
            if result:
                log_with_category(
                    ...
                    "GENERAL",
                    "info",
                    f"Track recognized from station {station_name} using AcoustID: {result.get('title')} by {result.get('artist')}",
                )
                return result
            else:
                log_with_category(
                    ...
                    "GENERAL",
                    "info",
                    f"No track recognized from station {station_name} using AcoustID, trying Audd",
                )

                # Try with Audd if AcoustID fails
                if self.audd_service and "raw_audio" in station_data:
                    log_with_category(
                        logger, "GENERAL", "info", f"Trying Audd for station {station_name}"
                    )
                    audd_result = await self.audd_service.detect_track_with_retry(
                        station_data["raw_audio"], max_retries=max_retries
                    )
                    ...
                    if audd_result:
                        log_with_category(
                            ...
                            "GENERAL",
                            "info",
                            f"Track recognized from station {station_name} using Audd: {audd_result.get('title')} by {audd_result.get('artist')}",
                        )
                        ...
                        if "detection" in audd_result and isinstance(
                            audd_result["detection"], dict
                        ):
                        ...
                        log_with_category(
                            ...
                            "GENERAL",
                            "info",
                            f"No track recognized from station {station_name} using Audd",
                        )
            ...
            log_with_category(
                ...
                "GENERAL",
                "error",
                f"Error recognizing track from station {station_name} using AcoustID: {str(e)}",
            )
            ...
            if self.audd_service and "raw_audio" in station_data:
                log_with_category(
                    ...
                    "GENERAL",
                    "info",
                    f"AcoustID failed with exception, trying Audd for station {station_name}",
                )
                try:
                    audd_result = await self.audd_service.detect_track_with_retry(
                        station_data["raw_audio"], max_retries=max_retries
                    ...
                    log_with_category(
                        ...
                        "GENERAL",
                        "error",
                        f"Error recognizing track from station {station_name} using Audd: {str(audd_error)}",
                    )
    ...
    async def search_recording(self, artist: str, title: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, lambda: musicbrainzngs.search_recordings(query=query, limit=5)
            )
            ...
            logger.info(
                f"MusicBrainz recording: {recording.get('title', 'Unknown')} by {recording.get('artist-credit', [{'artist': {'name': 'Unknown'}}])[0]['artist'].get('name', 'Unknown')}"
            )
                    ...
                    if release_id:
                        ...
                        release_info = await loop.run_in_executor(
                            ...
                            lambda: musicbrainzngs.get_release_by_id(
                                release_id, includes=["recordings", "isrcs", "labels"]
                            ),
                        )
                        ...
                        if "release" in release_info:
                                ...
                                for medium in release_data["medium-list"]:
                                    if "track-list" in medium:
                                        for track in medium["track-list"]:
                                            if (
                                                "recording" in track
                                                and "isrc-list" in track["recording"]
                                                and track["recording"]["isrc-list"]
                                            ):
                            ...
                            # Extract label
                            if (
                                "label-info-list" in release_data
                                and release_data["label-info-list"]
                            ):
            ...
            logger.info(
                f"Extracted from MusicBrainz - Title: {title}, Artist: {artist}, Album: {album}, ISRC: {isrc}, Label: {label}, Release date: {release_date}"
            )
            ...
            log_with_category(
                logger, "MUSICBRAINZ", "error", f"Traceback: {traceback.format_exc()}"
            )
            return None
