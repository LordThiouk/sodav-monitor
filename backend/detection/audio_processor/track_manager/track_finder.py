...
class TrackFinder:
    ...
    async def find_local_match(self, features: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not fingerprint:
                log_with_category(logger, "TRACK_FINDER", "warning", "No fingerprint found in features")
                return None
            ...
            if track:
                log_with_category(logger, "TRACK_FINDER", "info", f"Found exact fingerprint match: {track.title}")
            ...
            log_with_category(logger, "TRACK_FINDER", "info", "No exact match found, trying approximate match")
            
            # Récupérer toutes les pistes avec une empreinte digitale
            tracks_with_fingerprint = self.db_session.query(Track).filter(Track.fingerprint.isnot(None)).all()
            ...
            if best_match:
                log_with_category(logger, "TRACK_FINDER", "info", 
    ...
    def _create_match_result(self, track: Track, confidence: float, detection_method: str) -> Dict[str, Any]:
        """
