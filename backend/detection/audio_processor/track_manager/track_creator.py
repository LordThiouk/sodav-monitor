...
class TrackCreator:
    ...
    async def get_or_create_artist(self, artist_name: str) -> Optional[int]:
        ...
        try:
            ...
            if artist:
                log_with_category(logger, "TRACK_CREATOR", "debug", f"Artist found in database: {artist_name} (ID: {artist.id})")
                return artist.id
            ...
            log_with_category(logger, "TRACK_CREATOR", "info", f"Created new artist: {artist_name} (ID: {new_artist.id})")
    ...
    async def get_or_create_track(self, title: str, artist_id: int, album: Optional[str] = None, 
        ...
        try:
            ...
            if isinstance(self.db_session, MagicMock) and (not title or title == ""):
                log_with_category(logger, "TRACK_CREATOR", "warning", "Invalid track title, using 'Unknown Track'")
            ...
            if track:
                log_with_category(logger, "TRACK_CREATOR", "info", f"Track found in database: {title} (ID: {track.id})")
                ...
                if updated:
                    ...
                    log_with_category(logger, "TRACK_CREATOR", "info", f"Track updated: {title} (ID: {track.id})")
            ...
            log_with_category(logger, "TRACK_CREATOR", "info", f"New track created: {title} (ID: {track.id})")
