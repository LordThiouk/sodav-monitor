...
class StatsRecorder:
    ...
    def record_play_time(self, station_id: int, track_id: int, play_duration: float) -> bool:
        ...
        try:
            ...
            if isinstance(play_duration, (int, float)):
                ...
                log_with_category(logger, "STATS_RECORDER", "warning", 
            ...
            if not station:
                log_with_category(logger, "STATS_RECORDER", "warning", f"Station with ID {station_id} not found")
                return False
            
            # Vérifier que la piste existe
            track = self.db_session.query(Track).filter(Track.id == track_id).first()
            if not track:
                log_with_category(logger, "STATS_RECORDER", "warning", f"Track with ID {track_id} not found")
                return False
            ...
            try:
                ...
                log_with_category(logger, "STATS_RECORDER", "error", f"Error updating stats: {stats_error}")
    ...
    def _update_station_track_stats(self, station_id: int, track_id: int, play_duration: timedelta) -> None:
        """
        ...
        try:
            ...
            if stats:
                ...
                log_with_category(logger, "STATS_RECORDER", "debug", 
            ...
            log_with_category(logger, "STATS_RECORDER", "error", f"Error updating station track stats: {e}")
            # Ne pas faire de rollback ici, laisser la méthode appelante gérer la transaction
    
    def start_track_detection(self, track: Track, station_id: int, features: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            log_with_category(logger, "STATS_RECORDER", "error", f"Error starting track detection: {e}")
            return {"error": str(e)}
    ...
    def update_current_track(self, station_id: int, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            log_with_category(logger, "STATS_RECORDER", "error", f"Error updating current track: {e}")
            return {"error": str(e)}
    ...
    def record_detection(self, detection_result: Dict[str, Any], station_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not track_id:
                log_with_category(logger, "STATS_RECORDER", "error", "No track ID in detection result")
