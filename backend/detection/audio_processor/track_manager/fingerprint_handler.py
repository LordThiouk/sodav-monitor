...
class FingerprintHandler:
        ...
        try:
            ...
            if isinstance(audio_input, tuple) and len(audio_input) == 2:
                ...
                try:
                    ...
                    log_with_category(logger, "FINGERPRINT", "error", f"Error loading audio file: {e}")
                    return None
                ...
                try:
                    ...
                    log_with_category(logger, "FINGERPRINT", "error", f"Error loading audio from bytes: {e}")
                    return None
                    
            else:
                log_with_category(logger, "FINGERPRINT", "error", f"Unsupported audio input type: {type(audio_input)}")
                return None
    ...
    def _extract_features(self, audio_data: np.ndarray, sample_rate: int) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            try:
                ...
                log_with_category(logger, "FINGERPRINT", "warning", 
                                 "librosa not available, using mock features")
    ...
    def extract_fingerprint(self, features: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if not audio_data:
                log_with_category(logger, "FINGERPRINT", "warning", "No audio data available for fingerprint extraction")
                return None
            
            # Générer l'empreinte avec fpcalc
            fingerprint, _ = self.generate_acoustid_fingerprint(audio_data)
            if not fingerprint:
                log_with_category(logger, "FINGERPRINT", "warning", "Failed to generate fingerprint")
                return None
    ...
    def convert_features_to_audio(self, features: Dict[str, Any]) -> Optional[bytes]:
        ...
        try:
            ...
            if "mfcc" not in features and "spectrogram" not in features:
                log_with_category(logger, "FINGERPRINT", "warning", 
                                 "No audio features available for conversion")
                return None
            ...
            log_with_category(logger, "FINGERPRINT", "error", f"Error converting features to audio: {e}")
            return None
    ...
    def generate_acoustid_fingerprint(self, audio_data: bytes) -> Tuple[Optional[str], float]:
        ...
        try:
            ...
            log_with_category(logger, "FINGERPRINT", "error", f"Error generating AcoustID fingerprint: {e}")
            return None, 0
    ...
    def extract_audio_features(self, audio_data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Vérifier si les données audio sont disponibles
            if not audio_data:
                log_with_category(logger, "FINGERPRINT", "warning", "No audio data available for feature extraction")
                return None
            ...
            log_with_category(logger, "FINGERPRINT", "error", f"Error extracting audio features: {e}")
            return None
