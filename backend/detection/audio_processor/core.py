...
class AudioProcessor:
    ...
    async def process_stream(
        ...
        try:
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"Processing audio stream for station_id={station_id}",
            )
            ...
            if features is None:
                ...
                log_with_category(
                    ...
                    "AUDIO_PROCESSOR",
                    "info",
                    f"Stored raw audio in features: {len(raw_audio)} bytes",
                )

            log_with_category(
                logger, "AUDIO_PROCESSOR", "info", f"Features extracted: {features.keys()}"
            )

            # Récupérer la durée de lecture
            play_duration = features.get("play_duration", 0.0)
            log_with_category(
                logger, "AUDIO_PROCESSOR", "info", f"Play duration: {play_duration} seconds"
            )
            ...
            if not is_music:
                log_with_category(
                    ...
                    "AUDIO_PROCESSOR",
                    "info",
                    f"Speech detected for station_id={station_id}",
                )
                return {
                    "type": "speech",
                    "confidence": 0.0,
            ...
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"Attempting local detection for station_id={station_id}",
            )
            local_match = await self.track_manager.find_local_match(features)
            if local_match:
                log_with_category(
                    logger, "AUDIO_PROCESSOR", "info", f"Local match found: {local_match}"
                )
            ...
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"Local match not found, attempting MusicBrainz metadata detection for station_id={station_id}",
            )
            ...
            if musicbrainz_match:
                log_with_category(
                    ...
                    "AUDIO_PROCESSOR",
                    "info",
                    f"MusicBrainz metadata match found: {musicbrainz_match}",
                )
                # Ajouter la durée de lecture au résultat
            ...
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"MusicBrainz metadata match not found, attempting AcoustID detection for station_id={station_id}",
            )
            acoustid_match = await self.track_manager.find_acoustid_match(features)
            if acoustid_match:
                log_with_category(
                    logger, "AUDIO_PROCESSOR", "info", f"AcoustID match found: {acoustid_match}"
                )
                # Ajouter la durée de lecture au résultat
                    ...
                    try:
                        ...
                        if self.track_manager.db_session.dirty or self.track_manager.db_session.new:
                            ...
                            log_with_category(
                                ...
                                "AUDIO_PROCESSOR",
                                "info",
                                "Committed changes to database after AcoustID detection",
                            )
                    except Exception as e:
                        log_with_category(
                            logger, "AUDIO_PROCESSOR", "error", f"Error committing changes: {e}"
                        )
            ...
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"AcoustID match not found, attempting AudD detection for station_id={station_id}",
            )
            audd_match = await self.track_manager.find_audd_match(features)
            if audd_match:
                log_with_category(
                    logger, "AUDIO_PROCESSOR", "info", f"AudD match found: {audd_match}"
                )
                # Ajouter la durée de lecture au résultat
            ...
            log_with_category(
                logger, "AUDIO_PROCESSOR", "info", f"No match found for station_id={station_id}"
            )
            return {
                "type": "music",
                "source": "unknown",
            ...
            log_with_category(
                logger, "AUDIO_PROCESSOR", "error", f"Error processing audio stream: {e}"
            )
            return {"type": "error", "error": str(e), "station_id": station_id}
    ...
    async def start_monitoring(self, station_id: int) -> bool:
        ...
        try:
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"Starting monitoring for station_id={station_id}",
            )
            return await self.station_monitor.start_monitoring(
                self.stream_handler, self.feature_extractor, self.track_manager
            )
        except Exception as e:
            log_with_category(
                logger, "AUDIO_PROCESSOR", "error", f"Error starting monitoring: {str(e)}"
            )
            return False
    ...
    async def stop_monitoring(self, station_id: int) -> bool:
        ...
        try:
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"Stopping monitoring for station_id={station_id}",
            )
            return await self.station_monitor.stop_monitoring()
        except Exception as e:
            log_with_category(
                logger, "AUDIO_PROCESSOR", "error", f"Error stopping monitoring: {str(e)}"
            )
            return False
    ...
    async def detect_music(self, audio_features, station_id=None, metadata=None):
        ...
        try:
            log_with_category(
                logger, "AUDIO_PROCESSOR", "info", f"Detecting music for station_id={station_id}"
            )
            ...
            if not audio_features:
                log_with_category(
                    ...
                    "AUDIO_PROCESSOR",
                    "warning",
                    "No audio features provided for music detection",
                )
            ...
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"Attempting to find local match for station_id={station_id}",
            )
            ...
            if local_match and local_match.get("confidence", 0) >= self.local_match_threshold:
                ...
                if station_id and detection_id:
                    await self._update_stats(
                        station_id,
                        local_match.get("track_id"),
                        audio_features.get("play_duration", 0),
                    )
            ...
            log_with_category(
                ...
                "AUDIO_PROCESSOR",
                "info",
                f"Local match not found with sufficient confidence, attempting AcoustID detection for station_id={station_id}",
            )
            acoustid_match = await self.track_manager.find_acoustid_match(
                audio_features, station_id
            )
            ...
            return {"status": "no_match", "match_type": None, "track": None, "detection_id": None}

        except Exception as e:
            log_with_category(
                logger, "AUDIO_PROCESSOR", "error", f"Error detecting music: {str(e)}"
            )
            import traceback

            log_with_category(
                logger, "AUDIO_PROCESSOR", "error", f"Traceback: {traceback.format_exc()}"
            )
            return {"error": str(e)}
    ...
    async def _update_stats(self, station_id: int, track_id: int, play_duration: float):
        ...
        try:
            ...
            self.stats_updater.update_all_stats(
                detection_result, station_id, track, play_duration_td
            )
