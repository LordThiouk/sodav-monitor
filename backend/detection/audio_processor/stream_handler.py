...
class StreamHandler:
    ...
    def __init__(self, buffer_size: int = 4096, channels: int = 2):
        ...
        log_with_category(
            ...
            "STREAM",
            "info",
            f"StreamHandler initialized: buffer_size={buffer_size}, channels={channels}",
        )
    ...
    async def get_audio_data(self, stream_url: str) -> bytes:
        ...
        try:
            ...
            log_with_category(
                logger, "STREAM", "info", f"Attempting to get audio data from {stream_url}"
            )
            ...
            if response.status_code != 200:
                log_with_category(
                    ...
                    "STREAM",
                    "warning",
                    f"Failed to access stream {stream_url}, status code: {response.status_code}",
                )
                log_with_category(
                    logger, "STREAM", "info", "Generating synthetic audio data as fallback"
                )
                return self._generate_synthetic_audio()

            log_with_category(
                logger, "STREAM", "info", f"Successfully connected to {stream_url}, reading data"
            )
            ...
            if total_size == 0:
                log_with_category(
                    logger, "STREAM", "warning", f"No data received from {stream_url}"
                )
                log_with_category(
                    logger, "STREAM", "info", "Generating synthetic audio data as fallback"
                )
            ...
            log_with_category(
                logger, "STREAM", "info", f"Successfully read {total_size} bytes from {stream_url}"
            )
            ...
            log_with_category(
                logger, "STREAM", "info", f"Attempting to determine audio format from {stream_url}"
            )
            try:
                ...
                try:
                    ...
                    log_with_category(
                        logger, "STREAM", "info", f"Successfully loaded as MP3 from {stream_url}"
                    )
                    ...
                    try:
                        ...
                        log_with_category(
                            ...
                            "STREAM",
                            "info",
                            f"Successfully loaded as WAV from {stream_url}",
                        )
                        ...
                        try:
                            ...
                            log_with_category(
                                ...
                                "STREAM",
                                "info",
                                f"Successfully loaded as OGG from {stream_url}",
                            )
                            ...
                            try:
                                ...
                                log_with_category(
                                    ...
                                    "STREAM",
                                    "info",
                                    f"Successfully loaded as raw PCM from {stream_url}",
                                )
                                ...
                                log_with_category(
                                    ...
                                    "STREAM",
                                    "warning",
                                    f"Failed to determine audio format from {stream_url}: {raw_error}",
                                )
                                log_with_category(
                                    logger,
                                    "STREAM",
                                    "info",
                                    "Generating synthetic audio data as fallback",
                                )
                                return self._generate_synthetic_audio()

                # Convert to WAV format for processing
                log_with_category(
                    logger, "STREAM", "info", f"Converting audio to WAV format from {stream_url}"
                )
                wav_io = io.BytesIO()
                ...
                log_with_category(
                    logger, "STREAM", "info", f"Successfully processed audio data from {stream_url}"
                )
                return audio_bytes

            except ImportError:
                log_with_category(
                    logger, "STREAM", "warning", "pydub not installed, returning raw audio data"
                )
                return audio_bytes
            ...
            log_with_category(
                ...
                "STREAM",
                "error",
                f"Error getting audio data from stream {stream_url}: {str(e)}",
            )
            log_with_category(
                logger, "STREAM", "info", "Generating synthetic audio data due to error"
            )
            return self._generate_synthetic_audio()
