...
class FeatureExtractor:
    ...
    def __init__(
        self, sample_rate: int = 22050, n_mels: int = 128, n_fft: int = 2048, hop_length: int = 512
    ):
    ...
    def extract_features(self, audio_data: np.ndarray) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Calculated audio duration: {play_duration:.2f} seconds from {len(audio_data)} samples at {self.sample_rate} Hz"
        )
    ...
    def is_music(self, features: Dict[str, np.ndarray]) -> Tuple[bool, float]:
        ...
        try:
            ...
            if harmonic_ratio > 0.8 and rhythm_strength < 0.2:
                ...
                logger.info(
                    f"Adjusting rhythm strength from {rhythm_strength:.2f} to {adjusted_rhythm:.2f} due to high harmonic ratio"
                )
            ...
            primary_score = (
                0.5 * rhythm_strength
                + 0.5 * harmonic_ratio  # Rhythm is important  # Harmonic content equally important
            )

            secondary_score = (
                0.3 * (1.0 - spectral_flux)
                + 0.4 * timbre_stability  # Low flux indicates stability
                + 0.3 * chroma_variance  # Stable timbre helps  # Pitch variation helps
            )
            ...
            if rhythm_strength < 0.2 and harmonic_ratio < 0.2:  # Reduced thresholds
                ...
                logger.info(
                    f"Applied penalty: both primary features weak (rhythm={rhythm_strength:.2f}, harmonic={harmonic_ratio:.2f})"
                )
            elif rhythm_strength < 0.1 or harmonic_ratio < 0.1:  # Reduced thresholds
                base_confidence *= 0.8  # One primary feature very weak
                logger.info(
                    f"Applied penalty: one primary feature very weak (rhythm={rhythm_strength:.2f}, harmonic={harmonic_ratio:.2f})"
                )
            ...
            if harmonic_ratio > 0.8:
                ...
                logger.info(
                    f"Applied harmonic bonus: {harmonic_bonus:.2f}, adjusting confidence from {base_confidence:.2f} to {adjusted_confidence:.2f}"
                )
            ...
            logger.info(
                f"Final music detection: is_music={is_music}, confidence={base_confidence:.2f}"
            )
    ...
    def _calculate_rhythm_strength(self, mel_spectrogram: np.ndarray) -> float:
        ...
        try:
            ...
            logger.debug(
                f"Band shapes: bass={bass.shape}, low_mid={low_mid.shape}, mid={mid.shape}, high={high.shape}"
            )
            ...
            try:
                ...
                logger.debug(
                    f"Onset shapes: bass={bass_onsets.shape}, low_mid={low_mid_onsets.shape}, mid={mid_onsets.shape}, high={high_onsets.shape}"
                )
            ...
            logger.debug(
                f"Band scores: bass={bass_score:.4f}, low_mid={low_mid_score:.4f}, mid={mid_score:.4f}, high={high_score:.4f}"
            )
            ...
            logger.debug(
                f"Entropy scores: bass={bass_entropy:.4f}, low_mid={low_mid_entropy:.4f}, mid={mid_entropy:.4f}, high={high_entropy:.4f}, avg={avg_entropy:.4f}"
            )
            ...
            logger.info(
                f"Rhythm strength calculation: band_score={band_score:.4f}, tempo={tempo:.1f}, tempo_factor={tempo_factor:.1f}, entropy_score={entropy_score:.4f}, final={rhythm_strength:.4f}"
            )
    ...
    def _calculate_harmonic_ratio(self, mel_spectrogram: np.ndarray) -> float:
        ...
        try:
            if mel_spectrogram.shape[0] < 128:
                logger.warning(
                    f"Mel spectrogram too small for harmonic ratio: {mel_spectrogram.shape}"
                )
                return 0.0
            ...
            # Log band shapes for debugging
            logger.debug(
                f"Harmonic band shapes: low={low.shape}, mid_low={mid_low.shape}, mid_high={mid_high.shape}, high={high.shape}"
            )
            ...
            # Log band scores for debugging
            logger.debug(
                f"Harmonic band scores: low={low_score:.4f}, mid_low={mid_low_score:.4f}, mid_high={mid_high_score:.4f}, high={high_score:.4f}"
            )
            ...
            logger.debug(
                f"Harmonic relationships: low-mid_low={low_mid_low_score:.4f}, mid_low-mid_high={mid_low_mid_high_score:.4f}, mid_high-high={mid_high_high_score:.4f}"
            )
            ...
            # Log entropy scores
            logger.debug(
                f"Harmonic entropy scores: low={low_entropy:.4f}, mid_low={mid_low_entropy:.4f}, mid_high={mid_high_entropy:.4f}, high={high_entropy:.4f}, avg={avg_entropy:.4f}"
            )

            # Combine scores
            band_score = low_score + mid_low_score + mid_high_score + high_score
            relationship_score = (
                low_mid_low_score + mid_low_mid_high_score + mid_high_high_score
            ) / 3.0

            # Final harmonic ratio score
            harmonic_ratio = min(
                1.0, 0.5 * band_score + 0.3 * relationship_score + 0.2 * entropy_score
            )

            logger.info(
                f"Harmonic ratio calculation: band_score={band_score:.4f}, relationship_score={relationship_score:.4f}, entropy_score={entropy_score:.4f}, final={harmonic_ratio:.4f}"
            )
    ...
    def get_audio_duration(self, audio_data: np.ndarray) -> float:
        ...
        logger.debug(
            f"Calculated audio duration: {duration:.2f} seconds from {n_samples} samples at {self.sample_rate} Hz"
        )
