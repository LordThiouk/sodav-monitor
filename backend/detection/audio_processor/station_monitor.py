...
class StationMonitor:
    ...
    async def update_station_health(
        self, station: RadioStation, health_check: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            if health_check.get("is_available", False) and health_check.get(
                "is_audio_stream", False
            ):
                # Stream is healthy
    ...
    async def handle_station_recovery(self, station_id: int) -> bool:
        ...
        try:
            ...
            if health_check.get("is_available", False) and health_check.get(
                ...
                if (
                    station_id not in self.monitoring_tasks
                    or self.monitoring_tasks[station_id].done()
                ):
            ...
            logger.info(
                f"Recovery attempt for station {station.name} failed, stream still unhealthy"
            )
            return False
