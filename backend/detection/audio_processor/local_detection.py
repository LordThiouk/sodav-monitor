    ...
    async def detect(self, audio_data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            track = (
                ...
                .filter(func.similarity(Track.fingerprint, fingerprint) > self.min_confidence)
                .order_by(func.similarity(Track.fingerprint, fingerprint).desc())
                .first()
            )
            ...
            logger.info(
                f"Local detection successful: {track.title} by {artist_name} (confidence: {confidence:.2f})"
            )
    ...
    async def search_by_metadata(self, title: str, artist: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            artist_obj = (
                ...
                .filter(func.lower(Artist.name) == func.lower(artist))
                .first()
            )
            ...
            track = (
                self.db_session.query(Track)
                .filter(
                    func.lower(Track.title) == func.lower(title), Track.artist_id == artist_obj.id
                )
                .first()
            )
