...
from backend.models.models import (
    ...
    TrackStats, ArtistStats, StationTrackStats
)
...
class DBOperations:
    ...
    def get_or_create_track(self, title: str, artist_id: int, fingerprint: str,
                           fingerprint_raw: Optional[bytes] = None,
                           isrc: Optional[str] = None,
                           label: Optional[str] = None) -> Track:
        """
    ...
    def create_detection(self, track_id: int, station_id: int, confidence: float,
                        play_duration: timedelta, fingerprint: str,
                        audio_hash: Optional[str] = None) -> TrackDetection:
        """
    ...
    def _update_track_stats(self, track_id: int, confidence: float, play_duration: timedelta) -> None:
        """
        ...
        try:
            # Get track stats
            track_stats = self.db_session.query(TrackStats).filter(
    ...
    def _update_artist_stats(self, artist_id: int, confidence: float, play_duration: timedelta) -> None:
        """
        ...
        try:
            # Get artist stats
            artist_stats = self.db_session.query(ArtistStats).filter(
    ...
    def get_recent_detections(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            detections = self.db_session.query(
                TrackDetection, Track, Artist, RadioStation
            ).join(
