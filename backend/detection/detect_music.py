...
class MusicDetector:
    ...
    async def detect_music_from_station(self, station_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not station:
                log_with_category(
                    logger, "DETECTION", "error", f"Station with ID {station_id} not found"
                )
                log_with_category(
                    logger, "DETECTION", "error", f"Station with ID {station_id} not found"
                )
                return {"status": "error", "message": f"Station with ID {station_id} not found"}

            if not station.is_active:
                log_with_category(
                    logger, "DETECTION", "warning", f"Station {station.name} is not active"
                )
                log_with_category(
                    logger, "DETECTION", "warning", f"Station {station.name} is not active"
                )
                return {"status": "error", "message": f"Station {station.name} is not active"}

            if not station.stream_url:
                log_with_category(
                    logger, "DETECTION", "error", f"Station {station.name} has no stream URL"
                )
                return {"status": "error", "message": f"Station {station.name} has no stream URL"}
            ...
            log_with_category(
                ...
                "DETECTION",
                "info",
                f"Getting audio data from {station.name} ({station.stream_url})",
            )
            log_with_category(
                logger,
                "DETECTION",
            ...
            if audio_bytes is None or len(audio_bytes) == 0:
                log_with_category(
                    ...
                    "DETECTION",
                    "error",
                    f"[DETECTION] Failed to get audio data from {station.name}",
                )
                log_with_category(
                    logger,
                    "DETECTION",
            ...
            log_with_category(
                ...
                "DETECTION",
                "info",
                f"[DETECTION] Successfully retrieved {len(audio_bytes)} bytes of audio data from {station.name}",
            )
            ...
            log_with_category(
                ...
                "DETECTION",
                "info",
                f"[DETECTION] Processing result for {station.name}: {result}",
            )
            return result
            result = await self.process_audio_file(audio_bytes, station_id)
            log_with_category(
            ...
            log_with_category(
                ...
                "DETECTION",
                "error",
                f"[DETECTION] Error detecting music from station {station_id}: {str(e)}",
            )
            log_with_category(
                logger,
                "DETECTION",
    ...
    async def detect_music_from_all_stations(
        self, max_stations: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            stations = (
                self.db_session.query(RadioStation).filter(RadioStation.is_active == True).all()
            )
            ...
            if not stations:
                ...
                return {
                    ...
                    "details": {"total_stations": 0, "processed_stations": 0, "results": []},
                }
            ...
            log_with_category(
                logger, "DETECTION", "info", f"Processing {len(stations)} stations concurrently"
            )
            log_with_category(
                logger, "DETECTION", "info", f"Processing {len(stations)} stations concurrently"
            )
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    log_with_category(
                        ...
                        "DETECTION",
                        "error",
                        f"Error processing station {stations[i].name}: {result}",
                    )
                    log_with_category(
                        logger,
                        "DETECTION",
            ...
            log_with_category(
                logger, "DETECTION", "error", f"Error detecting music from all stations: {e}"
            )
            log_with_category(
                logger, "DETECTION", "error", f"Error detecting music from all stations: {e}"
            )
    ...
    async def process_audio_file(
        self, audio_data: bytes, station_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                log_with_category(
                    ...
                    "DETECTION",
                    "warning",
                    f"SoundFile failed to load audio: {sf_error}. Trying librosa...",
                )
                log_with_category(
                    logger,
                    "DETECTION",
                ...
                try:
                    ...
                    log_with_category(
                        ...
                        "DETECTION",
                        "error",
                        f"Both SoundFile and librosa failed to load audio: {librosa_error}",
                    )
                    log_with_category(
                        logger,
                        "DETECTION",
            ...
            if station_name:
                log_with_category(
                    logger, "DETECTION", "info", f"Processing audio data from {station_name}"
                )
                log_with_category(
                    logger, "DETECTION", "info", f"Processing audio data from {station_name}"
                )
            ...
            if result.get("type") == "speech":
                if station_name:
                    log_with_category(
                        logger, "DETECTION", "info", f"Speech detected on {station_name}"
                    )
                    log_with_category(
                        logger, "DETECTION", "info", f"Speech detected on {station_name}"
                    )
                ...
                if station_name:
                    log_with_category(
                        ...
                        "DETECTION",
                        "info",
                        f"Music detected on {station_name}: {track_title} by {track_artist}",
                    )
                    log_with_category(
                        logger,
                        "DETECTION",
                    ...
                    log_with_category(
                        ...
                        "DETECTION",
                        "info",
                        f"Music detected in audio file: {track_title} by {track_artist}",
                    )
                    log_with_category(
                        logger,
                        "DETECTION",
                ...
                if station_id:
                    try:
                        log_with_category(
                            ...
                            "DETECTION",
                            "info",
                            f"Attempting to record detection in database for station_id={station_id}",
                        )
                        log_with_category(
                            logger, "DETECTION", "info", f"Result structure: {result}"
                        )
                        log_with_category(
                            logger,
                            "DETECTION",
                            "info",
                            f"Recording detection regardless of track information",
                        )
                        ...
                        artist = (
                            ...
                            .filter(Artist.name == track_artist)
                            .first()
                        )
                        if not artist:
                            ...
                            log_with_category(
                                logger, "DETECTION", "info", f"Created new artist: {track_artist}"
                            )
                        ...
                        if not track:
                            ...
                            log_with_category(
                                ...
                                "DETECTION",
                                "info",
                                f"Creating new track: {track_title} by {track_artist}",
                            )
                            ...
                            log_with_category(
                                ...
                                "DETECTION",
                                "info",
                                f"Created new track: {track_title} by {track_artist}",
                            )
                        ...
                        log_with_category(
                            ...
                            "DETECTION",
                            "info",
                            f"Creating new detection for track_id={track.id}, station_id={station_id}",
                        )
                        detection = TrackDetection(
                            ...
                            play_duration=timedelta(
                                seconds=float(result.get("play_duration", 0.0))
                            ),
                        )
                        ...
                        log_with_category(
                            ...
                            "DETECTION",
                            "info",
                            f"Added detection to session, committing...",
                        )
                        ...
                        log_with_category(
                            ...
                            "DETECTION",
                            "info",
                            f"Recorded detection: {track_title} by {track_artist} on station {station_name}",
                        )
                        ...
                        log_with_category(
                            logger, "DETECTION", "error", f"Error recording detection: {str(e)}"
                        )
                        import traceback

                        log_with_category(
                            logger, "DETECTION", "error", f"Traceback: {traceback.format_exc()}"
                        )
                ...
                if station_name:
                    log_with_category(
                        ...
                        "DETECTION",
                        "warning",
                        f"Unknown content detected on {station_name}",
                    )
                    log_with_category(
                        logger,
                        "DETECTION",
                    ...
                    log_with_category(
                        logger, "DETECTION", "warning", "Unknown content detected in audio file"
                    )
                    log_with_category(
                        logger, "DETECTION", "warning", "Unknown content detected in audio file"
                    )
            ...
            log_with_category(
                logger, "DETECTION", "error", f"Error processing audio file: {str(e)}"
            )
            log_with_category(
                logger, "DETECTION", "error", f"Error processing audio file: {str(e)}"
            )
            return {"status": "error", "message": f"Error processing audio file: {str(e)}"}

    async def process_audio_data(
        self, station_id: int, audio_data: bytes, station_name: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result.get("success"):
                ...
                log_with_category(
                    ...
                    "DETECTION",
                    "info",
                    f"Track detected from {station_name}: {detection.get('title', 'Unknown')} by {detection.get('artist', 'Unknown')}",
                )
            else:
                log_with_category(
                    logger,
                    "DETECTION",
                    "info",
                    f"No track detected from {station_name}: {result.get('error', 'Unknown error')}",
                )
            ...
            log_with_category(
                logger, "DETECTION", "error", f"Error processing audio data: {str(e)}"
            )
            import traceback
