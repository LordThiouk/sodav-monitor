    ...
    def analyze_audio_features(self, audio_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                is_music_score = (
                    (sc_mean > 1000) * 0.4 + (rms_mean > 0.05) * 0.4 + (zcr_mean < 0.1) * 0.2
                )
            ...
            return {"is_music": False, "confidence": 0.0, "error": str(e)}
    ...
    async def recognize_from_audio_data(self, audio_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            if not features.get("is_music", False):
                ...
                return {"type": "speech", "confidence": features.get("confidence", 0.0)}
            ...
            if local_result and local_result.get("confidence", 0) >= self.min_confidence:
                logger.info(
                    f"Local detection successful: {local_result.get('title')} by {local_result.get('artist')}"
                )
                return {
                    "type": "music",
                    "source": "local",
            ...
            if mb_result and mb_result.get("confidence", 0) >= self.min_confidence:
                logger.info(
                    f"MusicBrainz detection successful: {mb_result.get('title')} by {mb_result.get('artist')}"
                )
                return {
                    "type": "music",
                    "source": "musicbrainz",
            ...
            if audd_result and audd_result.get("confidence", 0) >= self.min_confidence:
                logger.info(
                    f"AudD detection successful: {audd_result.get('title')} by {audd_result.get('artist')}"
                )
                return {
                    "type": "music",
                    "source": "audd",
