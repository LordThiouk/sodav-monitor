...
async def get_latest_detections(
    limit: int = 10, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Get the latest detections across all stations."""
    detections = (
        db.query(TrackDetection).order_by(TrackDetection.detected_at.desc()).limit(limit).all()
    )
