...
from backend.models.models import Artist, RadioStation, StationStatus, Track, TrackDetection
...
async def refresh_all_stations(
    ...
    stations = (
        ...
        .filter(RadioStation.status.in_([StationStatus.ACTIVE, StationStatus.ERROR]))
        .all()
    )
        ...
        detection_details.append(
            {
                ...
                "play_duration": detection.play_duration.total_seconds()
...
async def get_monitoring_stats(
    ...
    status_counts = {
        "active": db.query(RadioStation)
        .filter(RadioStation.status == StationStatus.ACTIVE)
        .count(),
        "inactive": db.query(RadioStation)
        .filter(RadioStation.status == StationStatus.INACTIVE)
        .count(),
        "error": db.query(RadioStation).filter(RadioStation.status == StationStatus.ERROR).count(),
        "maintenance": db.query(RadioStation)
        .filter(RadioStation.status == StationStatus.MAINTENANCE)
        .count(),
    }
    ...
    detection_count = (
        db.query(TrackDetection).filter(TrackDetection.detected_at >= yesterday).count()
    )
    ...
    return {
        ...
        "top_stations": [
            {"id": station.id, "name": station.name, "detection_count": station.detection_count}
