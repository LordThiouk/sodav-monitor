...
async def get_station_status(
    ...
    latest_status = db.query(StationStatusHistory).filter(
        StationStatusHistory.station_id == station_id
    ).order_by(StationStatusHistory.created_at.desc()).first()
...
async def get_status_summary(
    ...
    active_count = db.query(RadioStation).filter(RadioStation.status == StationStatus.ACTIVE).count()
    inactive_count = db.query(RadioStation).filter(RadioStation.status == StationStatus.INACTIVE).count()
    error_count = db.query(RadioStation).filter(RadioStation.status == StationStatus.ERROR).count()
    maintenance_count = db.query(RadioStation).filter(RadioStation.status == StationStatus.MAINTENANCE).count()
...
async def get_status_history(
    ...
    history = db.query(StationStatusHistory).filter(
        StationStatusHistory.station_id == station_id
    ).order_by(StationStatusHistory.created_at.desc()).limit(limit).all()
