...
async def get_subscription(
    ...
    subscription = (
        db.query(ReportSubscription).filter(ReportSubscription.id == subscription_id).first()
    )
...
async def list_subscriptions_by_email(
    email: EmailStr, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Get a list of report subscriptions for a specific email."""
    subscriptions = db.query(ReportSubscription).filter(ReportSubscription.email == email).all()
...
def calculate_next_delivery(frequency: str) -> datetime:
    ...
    if frequency == "daily":
        ...
        next_delivery = (now + timedelta(days=days_until_monday)).replace(
