...
def get_summary_stats(
    db: Session, report_type: str, start_date: datetime, end_date: datetime
) -> Dict:
    """Get summary statistics for a specific date range."""
    # Get total detections
    total_detections = (
        db.query(func.count(TrackDetection.id))
        .filter(TrackDetection.detected_at.between(start_date, end_date))
        .scalar()
    )
    ...
    # Get total play time
    total_play_time = (
        db.query(func.sum(TrackDetection.play_duration))
        .filter(TrackDetection.detected_at.between(start_date, end_date))
        .scalar()
    )
    ...
    # Get unique tracks
    unique_tracks = (
        db.query(func.count(distinct(TrackDetection.track_id)))
        .filter(TrackDetection.detected_at.between(start_date, end_date))
        .scalar()
    )
    ...
    # Get unique artists
    unique_artists = (
        db.query(func.count(distinct(Artist.id)))
    ...
    # Get unique stations
    unique_stations = (
        db.query(func.count(distinct(TrackDetection.station_id)))
        .filter(TrackDetection.detected_at.between(start_date, end_date))
        .scalar()
    )
