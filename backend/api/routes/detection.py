...
async def detect_music_all_stations(
    ...
    try:
        ...
        log_with_category(logger, "DETECTION", "error", f"Error detecting music on all stations: {str(e)}")
        return {
            "status": "error",
            "message": f"Error detecting music on all stations: {str(e)}"
        }
...
async def detect_station_music(station_id: int):
    ...
    try:
        ...
        log_with_category(logger, "DETECTION", "info", f"Music detection for station {station.name}: {result}")
        
    except Exception as e:
        log_with_category(logger, "DETECTION", "error", f"Error detecting music for station {station_id}: {str(e)}")
    finally:
        db.close() 